...
class acs_revisions {
	...
	*/
	...
	function __construct() {
		...
		add_filter('acs/validate_post_id', array($this, 'acs_validate_post_id'), 10, 2 );
	...
	*/
	...
	function wp_preview_post_fields( $fields ) {
		...
		if( acs_maybe_get_POST('wp-preview') !== 'dopreview' ) return $fields;
		
		
		// add to fields if ACS has changed
		if( acs_maybe_get_POST('_acs_changed') ) {
			
			$fields['_acs_changed'] = 'different than 1';
	...
	*/
	...
	function wp_save_post_revision_check_for_changes( $return, $last_revision, $post ) {
		...
		// if acs has changed, return false and prevent WP from performing 'compare' logic
		if( acs_maybe_get_POST('_acs_changed') ) return false;
	...
	*
	*  This filter will add the ACS fields to the returned array
	...
	*/
	...
	function wp_post_revision_fields( $fields, $post = null ) {
		...
		// validate page
		if( acs_is_screen('revision') || acs_is_ajax('get-revision-diffs') ) {
			
			// bail early if is restoring
			if( acs_maybe_get_GET('action') === 'restore' ) return $fields;
		...
		foreach( $meta as $name => $value ) {
			...
			$ancestors = acs_get_field_ancestors( $field );
			...
			if( !empty($ancestors) ) {
				...
				$oldest = acs_get_field( $ancestors[$count-1] );
		...
		if( !empty($append) ) {
			...
			// add prefix
			$append = acs_add_array_key_prefix($append, $prefix);
			$order = acs_add_array_key_prefix($order, $prefix);
			...
			// remove prefix
			$append = acs_remove_array_key_prefix($append, $prefix);
	...
	*/
	...
	function wp_post_revision_field( $value, $field_name, $post = null, $direction = false) {
		...
		// load field
		$field = acs_maybe_get_field( $field_name, $post_id );
	...
	*/
	...
	function wp_restore_post_revision( $post_id, $revision_id ) {
		...
		acs_copy_postmeta( $revision_id, $post_id );
		
		
		// Make sure the latest revision is also updated to match the new $post data
		// get latest revision
		$revision = acs_get_post_latest_revision( $post_id );
		...
		if( $revision ) {
			...
			acs_copy_postmeta( $revision_id, $revision->ID );
	...
	*/
	...
	function acs_validate_post_id( $post_id, $_post_id ) {
...
// initialize
acs()->revisions = new acs_revisions();

endif; // class_exists check


/*
*  acs_save_post_revision
*
...
function acs_save_post_revision( $post_id = 0 ) {
	...
	if( $revision ) {
		...
		acs_copy_postmeta( $post_id, $revision->ID );
...
function acs_get_post_latest_revision( $post_id ) {
