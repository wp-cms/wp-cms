...
// Register store.
acs_register_store( 'hook-variations' );

/**
 * acs_add_filter_variations
 *
 ...
 * @return	void
 */
function acs_add_filter_variations( $filter = '', $variations = array(), $index = 0 ) {
	
	// Store replacement data.
	acs_get_store('hook-variations')->set( $filter, array(
		'type'			=> 'filter',
	...
	add_filter( $filter, '_acs_apply_hook_variations', 10, 10 );
}

/**
 * acs_add_action_variations
 *
 ...
 * @return	void
 */
function acs_add_action_variations( $action = '', $variations = array(), $index = 0 ) {
	
	// Store replacement data.
	acs_get_store('hook-variations')->set( $action, array(
		'type'			=> 'action',
		'variations' 	=> $variations,
		'index' 		=> $index,
	));
	...
	// Add generic handler.
	// Use a priotiry of 10, and accepted args of 10 (ignored by WP).
	add_action( $action, '_acs_apply_hook_variations', 10, 10 );
}

/**
 * _acs_apply_hook_variations
 *
 ...
 * @return	mixed
 */
function _acs_apply_hook_variations() {
	...
	$variations = acs_get_store('hook-variations')->get( $filter );
	extract( $variations );
...
// Register store.
acs_register_store( 'deprecated-hooks' );

/**
 * acs_add_deprecated_filter
 *
 ...
 * @return	void
 */
function acs_add_deprecated_filter( $deprecated, $version, $replacement ) {
	
	// Store replacement data.
	acs_get_store('deprecated-hooks')->append(array(
		'type'			=> 'filter',
	...
	// Add generic handler.
	// Use a priotiry of 10, and accepted args of 10 (ignored by WP).
	add_filter( $replacement, '_acs_apply_deprecated_hook', 10, 10 );
}

/**
 * acs_add_deprecated_action
 *
 ...
 * @return	void
 */
function acs_add_deprecated_action( $deprecated, $version, $replacement ) {
 ...
 * @return	mixed
 */
function _acs_apply_deprecated_hook() {
	...
	$items = acs_get_store('deprecated-hooks')->query(array( 'replacement' => $hook ));
