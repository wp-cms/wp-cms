...
if( ! class_exists('ACS_Compatibility') ) :

class ACS_Compatibility {
	...
	*/
	function __construct() {
		...
		// actions
		add_filter('acs/validate_field',						array($this, 'validate_field'), 20, 1);
		add_filter('acs/validate_field/type=textarea',			array($this, 'validate_textarea_field'), 20, 1);
		add_filter('acs/validate_field/type=relationship',		array($this, 'validate_relationship_field'), 20, 1);
		add_filter('acs/validate_field/type=post_object',		array($this, 'validate_relationship_field'), 20, 1);
		add_filter('acs/validate_field/type=page_link',			array($this, 'validate_relationship_field'), 20, 1);
		add_filter('acs/validate_field/type=image',				array($this, 'validate_image_field'), 20, 1);
		add_filter('acs/validate_field/type=file',				array($this, 'validate_image_field'), 20, 1);
		add_filter('acs/validate_field/type=wysiwyg',			array($this, 'validate_wysiwyg_field'), 20, 1);
		add_filter('acs/validate_field/type=date_picker',		array($this, 'validate_date_picker_field'), 20, 1);
		add_filter('acs/validate_field/type=taxonomy',			array($this, 'validate_taxonomy_field'), 20, 1);
		add_filter('acs/validate_field/type=date_time_picker',	array($this, 'validate_date_time_picker_field'), 20, 1);
		add_filter('acs/validate_field/type=user',				array($this, 'validate_user_field'), 20, 1);
		add_filter('acs/validate_field_group',					array($this, 'validate_field_group'), 20, 1);
		
		// Modify field wrapper attributes
		add_filter('acs/field_wrapper_attributes',				array($this, 'field_wrapper_attributes'), 20, 2);
		
		// location
		add_filter('acs/location/validate_rule/type=post_taxonomy', array($this, 'validate_post_taxonomy_location_rule'), 20, 1);
		add_filter('acs/location/validate_rule/type=post_category', array($this, 'validate_post_taxonomy_location_rule'), 20, 1);
		
		// Update settings
		add_action('acs/init', array($this, 'init'));
	}
	...
	function init() {
		...
		if( defined('ACS_LITE') && ACS_LITE ) {
			acs_update_setting( 'show_admin', false );
		}
	}
	...
	function field_wrapper_attributes( $wrapper, $field ) {
		...
		if( acs_get_compatibility('field_wrapper_class') ) {
	...
	function validate_field( $field ) {
		...
		if( isset($field['conditional_logic']['status']) ) {
			...
			if( $field['conditional_logic']['status'] ) {
				$field['conditional_logic'] = acs_convert_rules_to_groups($field['conditional_logic']['rules'], $field['conditional_logic']['allorany']);
			} else {
	...
	*/
	function validate_textarea_field( $field ) {
		...
		$formatting = acs_extract_var( $field, 'formatting' );
		if( $formatting === 'br' ) {
			$field['new_lines'] = 'br';
		}
	...
	*/
	function validate_image_field( $field ) {
		...
		if( isset($field['save_format']) ) {
			$field['return_format'] = acs_extract_var( $field, 'save_format' );
		}
	...
	*/
	function validate_date_picker_field( $field ) {
		...
		if( isset($field['date_format']) ) {
			...
			$display_format = acs_convert_date_to_php( $display_format );
	...
	*/
	function validate_taxonomy_field( $field ) {
		...
		if( isset($field['load_save_terms']) ) {
			$field['save_terms'] = acs_extract_var( $field, 'load_save_terms' );
		}
	...
	*/
	function validate_date_time_picker_field( $field ) {
		...
		// https://github.com/soderlind/acs-field-date-time-picker
		if( !empty($field['time_format']) ) {
			
			// extract vars
			$time_format = acs_extract_var( $field, 'time_format' );
			$date_format = acs_extract_var( $field, 'date_format' );
			$get_as_timestamp = acs_extract_var( $field, 'get_as_timestamp' );
			
			// convert from js to php
			$time_format = acs_convert_time_to_php( $time_format );
			$date_format = acs_convert_date_to_php( $date_format );
	...
	*/
	function validate_user_field( $field ) {
		...
		if( acs_in_array('all', $field['role']) ) {
			$field['role'] = '';
		}
		...
		if( isset($field['field_type']) ) {
			...
			// extract vars
			$field_type = acs_extract_var( $field, 'field_type' );
	...
	*
	*  This function will provide compatibility with ACS4 field groups
	*
	*  @type	function
	*  @date	23/04/2014
	*  @since	5.0.0
	...
	*/
	function validate_field_group( $field_group ) {
		...
		// detect ACS4 data and generate key
		if( !$field_group['key'] ) {
			$version = 4;
		...
		if( isset($field_group['options']) ) {
			$options = acs_extract_var($field_group, 'options');
		...
		if( isset($field_group['location']['rules']) ) {
			$field_group['location'] = acs_convert_rules_to_groups($field_group['location']['rules'], $field_group['location']['allorany']);
		}
		...
		$replace = array(
	 	...
	 	// only replace 'taxonomy' rule if is an ACS4 field group
	 	if( $version > 4 ) {
		...
		if( isset($field_group['layout']) ) {
			$field_group['style'] = acs_extract_var($field_group, 'layout');
		}
	...
	*/
	function validate_post_taxonomy_location_rule( $rule ) {
		...
		// previous versions of ACS (v4.4.12) saved value as term_id
		// convert term_id into "taxonomy:slug" string
		if( is_numeric($rule['value']) ) {
			$term = acs_get_term( $rule['value'] );
			if( $term ) {
				$rule['value'] = acs_encode_term($term);
			}
		}
...
acs_new_instance('ACS_Compatibility');

endif; // class_exists check

/*
 * acs_get_compatibility
 *
 ...
 * @return	bool
 */
function acs_get_compatibility( $name ) {
	return apply_filters( "acs/compatibility/{$name}", false );
}