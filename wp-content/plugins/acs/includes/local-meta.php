...
if( ! class_exists('ACS_Local_Meta') ) :

class ACS_Local_Meta {
	...
	function __construct() {
		...
		// add filters
		add_filter( 'acs/pre_load_post_id', 	array($this, 'pre_load_post_id'), 	1, 2 );
		add_filter( 'acs/pre_load_meta', 		array($this, 'pre_load_meta'), 		1, 2 );
		add_filter( 'acs/pre_load_metadata', 	array($this, 'pre_load_metadata'), 	1, 4 );
	}
	...
	function is_request( $meta = array() ) {
		return acs_is_field_key( key( $meta ) );
	}
	...
	function capture( $values = array(), $post_id = 0 ) {
		...
		add_filter('acs/pre_update_metadata', array($this, 'capture_update_metadata'), 1, 5);
		
		// Simulate update.
		if( $values ) {
			acs_update_values( $values, $post_id );
		}
		
		// Remove listener filter.
		remove_filter('acs/pre_update_metadata', array($this, 'capture_update_metadata'), 1, 5);
 ...
 * @date	8/10/18
 * @since	5.8.0
 * @see		ACS_Local_Meta::add() for list of parameters.
 *
 * @return	array
 */
function acs_setup_meta( $meta = array(), $post_id = 0, $is_main = false ) {
	return acs_get_instance('ACS_Local_Meta')->add( $meta, $post_id, $is_main );
}
 ...
 * @date	8/10/18
 * @since	5.8.0
 * @see		ACS_Local_Meta::remove() for list of parameters.
 *
 * @return	void
 */
function acs_reset_meta( $post_id = 0 ) {
	return acs_get_instance('ACS_Local_Meta')->remove( $post_id );
}
