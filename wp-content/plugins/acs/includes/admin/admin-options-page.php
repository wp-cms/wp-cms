...
if( ! class_exists('acs_admin_options_page') ) :

class acs_admin_options_page {
	...
	*/
	...
	function admin_load() {
		...
		// vars
		$this->page = acs_get_options_page( $plugin_page );
		
		
		// get post_id (allow lang modification)
		$this->page['post_id'] = acs_get_valid_post_id($this->page['post_id']);
		
		
		// verify and remove nonce
		if( acs_verify_nonce('options') ) {
		
			// save data
		    if( acs_validate_save_post(true) ) {
		    	
		    	// set autoload
		    	acs_update_setting('autoload', $this->page['autoload']);
		    	
		    	
		    	// save
				acs_save_post( $this->page['post_id'] );
	...
	*/
	...
	function admin_head() {
		...
		if( !empty($_GET['message']) && $_GET['message'] == '1' ) {
			acs_add_admin_notice( $this->page['updated_message'], 'success' );
		}
		
		
		// add submit div
		add_meta_box('submitdiv', __('Publish','acs'), array($this, 'postbox_submitdiv'), 'acs_options_page', 'side', 'high');
		
		
		
		if( empty($field_groups) ) {
			
			acs_add_admin_notice( sprintf( __('No Custom Field Groups found for this options page. <a href="%s">Create a Custom Field Group</a>', 'acs'), admin_url('post-new.php?post_type=acs-field-group') ), 'warning' );
			...
			foreach( $field_groups as $i => $field_group ) {
				...
				$priority = apply_filters('acs/input/meta_box_priority', $priority, $field_group);
				
				
				// add meta box
				add_meta_box( $id, $title, array($this, 'postbox_acs'), 'acs_options_page', $context, $priority, $args );
	...
	*/
	...
	function postbox_submitdiv( $post, $args ) {
		...
		*/
		do_action( 'acs/options_page/submitbox_before_major_actions', $this->page );
		?>
		<div id="major-publishing-actions">
			...
			*  @param array $page The current options page.
			*/
			do_action( 'acs/options_page/submitbox_major_actions', $this->page );
			?>
			<div class="clear"></div>
	...
	*/
	...
	function postbox_acs( $post, $args ) {
		...
		$o = array(
			...
			'editLink'		=> '',
			'editTitle'		=> __('Edit field group', 'acs'),
			'visibility'	=> true
		);
		
		
		// edit_url
		if( $field_group['ID'] && acs_current_user_can_admin() ) {
		...
		// render
		acs_render_fields( $fields, $this->page['post_id'], 'div', $field_group['instruction_placement'] );
...
if( typeof acs !== 'undefined' ) {
	...
	acs.newPostbox(<?php echo json_encode($o); ?>);
...
<?php
	...
	*/
	...
	function html() {
		...
		// load view
		acs_get_view(dirname(__FILE__) . '/views/html-options-page.php', $this->page);
