...
// Vars.
$parent_slug = 'edit.php?post_type=acs-field-group';
...
if( isset($submenu[ $parent_slug ]) ) {
	foreach( $submenu[ $parent_slug ] as $i => $sub_item ) {
		...
		if( $i === 0 && $submenu_file === 'post-new.php?post_type=acs-field-group' ) {
			$tab['is_active'] = true;
		}
		$tabs[] = $tab;
	}
 ...
 */
$tabs = apply_filters( 'acs/admin/toolbar', $tabs );
...
?>
<div class="acs-admin-toolbar">
	<h2><i class="acs-tab-icon dashicons dashicons-welcome-widgets-menus"></i> <?php echo acs_get_setting('name'); ?></h2>
	<?php foreach( $tabs as $tab ) {
		printf(
			'<a class="acs-tab%s" href="%s">%s</a>',
			!empty( $tab['is_active'] ) ? ' is-active' : '',
			esc_url( $tab['url'] ),
			acs_esc_html( $tab['text'] )
		);
	} ?>
</div>