...
if( ! class_exists('ACS_Admin_Field_Groups') ) :

class ACS_Admin_Field_Groups {
	...
	public function get_admin_url( $params = '' ) {
		return admin_url( "edit.php?post_type=acs-field-group{$params}" );
	}
	...
	/**
	 * Redirects users from ACS 4.0 admin page.
	 *
	 * @date	17/9/18
	 * @since	5.7.6
	 *
	...
	public function handle_redirection() {
		if( isset($_GET['post_type']) && $_GET['post_type'] === 'acs' ) {
	...
	public function current_screen() {
		...
		if( !acs_is_screen('edit-acs-field-group') ) {
			return;
		}
		...
		$wp_post_statuses['publish']->label_count = _n_noop( 'Active <span class="count">(%s)</span>', 'Active <span class="count">(%s)</span>', 'acs' );
		$wp_post_statuses['trash'] = acs_extract_var( $wp_post_statuses, 'trash' );
		
		// Add hooks.
		add_action( 'admin_enqueue_scripts',						array( $this, 'admin_enqueue_scripts') );
		add_action( 'admin_body_class', 							array( $this, 'admin_body_class' ) );
		add_filter( 'views_edit-acs-field-group',					array( $this, 'admin_table_views' ), 10, 1 );
		add_filter( 'manage_acs-field-group_posts_columns',			array( $this, 'admin_table_columns' ), 10, 1 );
		add_action( 'manage_acs-field-group_posts_custom_column',	array( $this, 'admin_table_columns_html' ), 10, 2 );
		add_filter( 'display_post_states',							array( $this, 'display_post_states' ), 10, 2 );
		add_filter( 'bulk_actions-edit-acs-field-group',			array( $this, 'admin_table_bulk_actions' ), 10, 1 );
	...
	public function setup_sync() {
		...
		if( acs_get_local_json_files() ) {
			
			// Get all groups in a single cached query to check if sync is available.
			$all_field_groups = acs_get_field_groups();
			foreach( $all_field_groups as $field_group ) {
				
				// Extract vars.
				$local = acs_maybe_get( $field_group, 'local' );
				$modified = acs_maybe_get( $field_group, 'modified' );
				$private = acs_maybe_get( $field_group, 'private' );
	...
	public function admin_enqueue_scripts() {
		acs_enqueue_script( 'acs' );
		
		// Localize text.
		acs_localize_text(array(
			'Review local JSON changes' => __( 'Review local JSON changes', 'acs' ),
			'Loading diff'              => __( 'Loading diff', 'acs' ),
			'Sync changes'              => __( 'Sync changes', 'acs' ),
		));
	}
	...
	public function admin_body_class( $classes ) {
		$classes .= ' acs-admin-field-groups';
		if( $this->view ) {
	...
	public function get_disabled_post_state() {
		return '<span class="dashicons dashicons-hidden"></span> ' . _x( 'Disabled', 'post status', 'acs' );
	}
	...
	public function display_post_states( $post_states, $post ) {
		if( $post->post_status === 'acs-disabled' ) {
			$post_states['acs-disabled'] = $this->get_disabled_post_state();
		}
		return $post_states;
	}
	...
	public function admin_table_columns( $_columns ) {
		$columns = array(
			...
			'acs-description'	=> __('Description', 'acs'),
			'acs-key'			=> __('Key', 'acs'),
			'acs-location' 		=> __('Location', 'acs'),
			'acs-count' 		=> __('Fields', 'acs'),
		);
		if( acs_get_local_json_files() ) {
			$columns['acs-json'] = __('Local JSON', 'acs');
		}
		return $columns;
	}
	...
	public function admin_table_columns_html( $column_name, $post_id ) {
		$field_group = acs_get_field_group( $post_id );
		if( $field_group ) {
	...
	public function render_admin_table_column( $column_name, $field_group ) {
		switch ( $column_name ) {
			...
			// Description.
			case 'acs-description':
				if( $field_group['description'] ) {
					echo '<span class="acs-description">' . acs_esc_html( $field_group['description'] ) . '</span>';
				}
				break;
			...
			// Count.
			case 'acs-count':
				echo esc_html( acs_get_field_count( $field_group ) );
				break;
	...
	public function render_admin_table_column_locations( $field_group ) {
		...
		if( $field_group['location'] ) {
			foreach( $field_group['location'] as $i => $rules ) {
				...
				foreach( $rules as $j => $rule ) {
					...
					$location = acs_get_location_rule( $rule['param'] );
				...
				$object_subtypes = array_map('acs_array', $object_subtypes);
				...
				foreach( $object_subtypes as $object_subtype ) {
					$object = acs_get_object_type( $object_type, $object_subtype );
					if( $object ) {
		...
		if( $objects ) {
			...
			// Icon.
			$html .= '<span class="dashicons ' . $objects[0]->icon . ($total > 1 ? ' acs-multi-dashicon' : '') . '"></span> ';
			...
			$html = '<span class="dashicons dashicons-businesswoman"></span> ' . __( 'Various', 'acs' );
		}
		
		// Filter.
		echo acs_esc_html( $html );
	}
	...
	public function get_human_readable_file_location( $file ) {
		...
		if( strpos($file, $theme_path) !== false ) {
			...
			$located = sprintf( __('Located in theme: %s', 'acs'), $rel_file );
			
		} elseif( strpos($file, WP_PLUGIN_DIR) !== false ) {
			$rel_file = str_replace( WP_PLUGIN_DIR, '', $file );
			$located = sprintf( __('Located in plugin: %s', 'acs'), $rel_file );
		
		} else {
			$rel_file = str_replace( ABSPATH, '', $file );
			$located = sprintf( __('Located in: %s', 'acs'), $rel_file );
		}
		return $located;
	}
	...
	public function render_admin_table_column_local_status( $field_group ) {
		$json = acs_get_local_json_files();
		if( isset( $json[ $field_group['key'] ] ) ) {
			$file = $json[ $field_group['key'] ];
			if( isset($this->sync[ $field_group['key'] ]) ) {
				$url = $this->get_admin_url( '&acssync=' . $field_group['key'] . '&_wpnonce=' . wp_create_nonce('bulk-posts') );
				echo '<strong>' . __( 'Sync available', 'acs' ) . '</strong>';
				if( $field_group['ID'] ) {
					echo '<div class="row-actions">
						<span class="sync"><a href="' . esc_url( $url ) . '">' . __( 'Sync', 'acs' ) . '</a> | </span>
						<span class="review"><a href="#" data-event="review-sync" data-id="' . esc_attr($field_group['ID']) . '" data-href="' . esc_url( $url ) . '">' . __( 'Review changes', 'acs' ) . '</a></span>
					</div>';
				} else {
					echo '<div class="row-actions">
						<span class="sync"><a href="' . esc_url( $url ) . '">' . __( 'Import', 'acs' ) . '</a></span>
					</div>';
				}
			} else {
				echo __( 'Saved', 'acs' );
			...
			echo '<span class="acs-secondary-text">' . __( 'Awaiting save', 'acs' ) . '</span>';
		}
	}
	...
	public function page_row_actions( $actions, $post ) {
		...
		$duplicate_action_url = $this->get_admin_url( '&acsduplicate=' . $post->ID . '&_wpnonce=' . wp_create_nonce('bulk-posts') );
		$actions[ 'acsduplicate' ] = '<a href="' . esc_url( $duplicate_action_url ) . '" aria-label="' . esc_attr__( 'Duplicate this item', 'acs' ) . '">' . __( 'Duplicate', 'acs' ) . '</a>';
		
		// Return actions in custom order.
		$order = array( 'edit', 'acsduplicate', 'trash' );
	...
	public function admin_table_bulk_actions( $actions ) {
		...
		if( $this->view !== 'sync' ) {
			$actions[ 'acsduplicate' ] = __( 'Duplicate', 'acs' );
		}
		...
		if( $this->sync ) {
			...
			$actions[ 'acssync' ] = __( 'Sync changes', 'acs' );
		}
		return $actions;
	}
	...
	public function check_duplicate() {
		...
		if( isset($_GET['acsduplicatecomplete']) ) {
			$ids = array_map( 'intval', explode(',', $_GET['acsduplicatecomplete']) );
			
			// Generate text.
			$text = sprintf( 
				_n( 'Field group duplicated.', '%s field groups duplicated.', count($ids), 'acs' ),
				count($ids)
			);
		...
		$ids = array();
		if( isset($_GET['acsduplicate']) ) {
			$ids[] = intval( $_GET['acsduplicate'] );
		} elseif( isset($_GET['post'], $_GET['action2']) && $_GET['action2'] === 'acsduplicate' ) {
		...
		if( $ids ) {
			...
			$new_ids = array();
			foreach( $ids as $id ) {
				$field_group = acs_duplicate_field_group( $id );
				$new_ids[] = $field_group['ID'];
			}
			
			// Redirect.
			wp_redirect( $this->get_admin_url( '&acsduplicatecomplete=' . implode(',', $new_ids) ) );
			exit;
		}
	}
	
	/**
	 * Checks for the custom "acssync" action.
	 *
	 * @date	15/4/20
	 * @since	5.9.0
	 *
	...
	public function check_sync() {
		...
		// Display notice on success redirect.
		if( isset($_GET['acssynccomplete']) ) {
			$ids = array_map( 'intval', explode(',', $_GET['acssynccomplete']) );
			
			// Generate text.
			$text = sprintf( 
				_n( 'Field group synchronised.', '%s field groups synchronised.', count($ids), 'acs' ),
				count($ids)
			);
		...
		// Find items to sync.
		$keys = array();
		if( isset($_GET['acssync']) ) {
			$keys[] = sanitize_text_field( $_GET['acssync'] );
		} elseif( isset($_GET['post'], $_GET['action2']) && $_GET['action2'] === 'acssync' ) {
		...
		if( $keys && $this->sync ) {
			...
			foreach( $this->sync as $key => $field_group ) {
				...
				$result = acs_import_field_group( $local_field_group );
				$new_ids[] = $result['ID'];
			}
			
			// Redirect.
			wp_redirect( $this->get_current_admin_url( '&acssynccomplete=' . implode(',', $new_ids) ) );
			exit;
		}
	}
	...
	public function admin_table_views( $views ) {
		...
		if( $count ) {
			$views['sync'] = sprintf(
				...
				esc_html( __('Sync available', 'acs') ),
				$count
			);
		}
...
(function($){
	...
	function reviewSync( props ) {
		var modal = acs.newModal({
			title: acs.__('Review local JSON changes'),
			content: '<p class="acs-modal-feedback"><i class="acs-loading"></i> ' + acs.__('Loading diff') + '</p>',
			toolbar: '<a href="' + props.href + '" class="button button-primary button-sync-changes disabled">' + acs.__('Sync changes') + '</a>',
		});
		...
		var xhr = $.ajax({
			...
			method: 'POST',
			dataType: 'json',
			data: acs.prepareForAjax({
				action:	'acs/ajax/local_json_diff',
				id: props.id
			})
		})
		...
		.fail(function( jqXHR, textStatus, errorThrown ) {
			if( error = acs.getXhrError(jqXHR) ) {
				modal.content( '<p class="acs-modal-feedback error">' + error + '</p>' );
			}
		});
	}
	...
	foreach( $this->sync as $k => $field_group ) {
		...
		foreach( $columns as $column_name => $column_label ) {
			...
			switch ( $column_name ) {
				...
				// Checkbox.
				case 'cb':
					echo '<label for="cb-select-' . esc_attr( $k ) .'" class="screen-reader-text">' .esc_html( sprintf( __( 'Select %s', 'acs' ), $field_group['title'] ) ) . '</label>';
				...
				case 'title':
					...
					echo '<div class="row-actions"><span class="file acs-secondary-text">' . $this->get_human_readable_file_location( $field_group['local_file'] ) . '</span></div>';
...
<script type="text/javascript">
(function($){
	$('#the-list').html( $('#acs-the-list').children() );
})(jQuery);
</script>
		<?php
	}
	...
	public function trashed_post( $post_id ) {
		if( get_post_type($post_id) === 'acs-field-group' ) {
			acs_trash_field_group( $post_id );
		}
	}
	...
	public function untrashed_post( $post_id ) {
		if( get_post_type($post_id) === 'acs-field-group' ) {
			acs_untrash_field_group( $post_id );
		}
	}
	...
	public function deleted_post( $post_id ) {
		if( get_post_type($post_id) === 'acs-field-group' ) {
			acs_delete_field_group( $post_id );
		}
	}	
}

// Instantiate.
acs_new_instance('ACS_Admin_Field_Groups');
