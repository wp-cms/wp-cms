 ...
 * @return	object
 */
function acs_get_object_type( $object_type, $object_subtype = '' ) {
	$props = array(
		'type'		=> $object_type,
	...
	switch ( $object_type ) {
		case 'post':
			if( $object_subtype ) {
				...
				if( $post_type ) {
					...
					$props['icon'] = acs_with_default( $post_type->menu_icon, 'dashicons-admin-post' );
				} else {
					return false;
				}
			} else {
				$props['label'] = __('Posts', 'acs');
		...
		case 'term':
			if( $object_subtype ) {
				...
				$props['label'] = __('Taxonomies', 'acs');
			}
			$props['icon'] = 'dashicons-tag';
			break;
		case 'attachment':
			$props['label'] = __('Attachments', 'acs');
			$props['icon'] = 'dashicons-admin-media';
			break;
		case 'comment':
			$props['label'] = __('Comments', 'acs');
			$props['icon'] = 'dashicons-admin-comments';
			break;
		case 'widget':
			$props['label'] = __('Widgets', 'acs');
			$props['icon'] = 'dashicons-screenoptions';
			break;
		case 'menu':
			$props['label'] = __('Menus', 'acs');
			$props['icon'] = 'dashicons-admin-appearance';
			break;
		case 'menu_item':
			$props['label'] = __('Menu items', 'acs');
			$props['icon'] = 'dashicons-admin-appearance';
			break;
		case 'user':
			$props['label'] = __('Users', 'acs');
			$props['icon'] = 'dashicons-admin-users';
			break;
		case 'option':
			$props['label'] = __('Options', 'acs');
			$props['icon'] = 'dashicons-admin-generic';
			break;
		case 'block':
			$props['label'] = __('Blocks', 'acs');
			$props['icon'] = acs_version_compare('wp', '>=', '5.5') ? 'dashicons-block-default' : 'dashicons-layout';
			break;
		default:
			return false;
	}
	 ...
	 * @param	string $object_subtype Optional object subtype (post type, taxonomy).
	 */
	return apply_filters( 'acs/get_object_type', $object, $object_type, $object_subtype );
}
 ...
 * @return	array
 */
function acs_decode_post_id( $post_id = 0 ) {
	$type = '';
	$id = 0;
	 ...
	 * @param	(int|string) $post_id The post id.
	 */
	return apply_filters( "acs/decode_post_id", compact( 'type', 'id' ), $post_id );
}
