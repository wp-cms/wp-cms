...
/*
*  ACS Taxonomy Form Class
*
*  All the logic for adding fields to taxonomy terms
*
*  @class 		acs_form_taxonomy
*  @package		ACS
*  @subpackage	Forms
*/

if( ! class_exists('acs_form_taxonomy') ) :
	...
	*/
	...
	function add_term( $taxonomy ) {
		...
		if( !empty($field_groups) ) {
			...
			// wrap
			echo '<div id="acs-term-fields" class="acs-fields -clear">';
			
			// loop
			foreach( $field_groups as $field_group ) {
				$fields = acs_get_fields( $field_group );
				acs_render_fields( $fields, $post_id, 'div', 'field' );
			}
		...
		if( !empty($field_groups) ) {
			...
			foreach( $field_groups as $field_group ) {
				...
				echo '<table class="form-table">';
					$fields = acs_get_fields( $field_group );
					acs_render_fields( $fields, $post_id, 'tr', 'field' );
				echo '</table>';
...
if( $this->view == 'add' ): ?>
	...
	// vars
	var $fields = $('#acs-term-fields');
	var html = '';
	
	// Store a copy of the $fields html used later to replace after AJAX request.
	// Hook into 'prepare' action to allow ACS core helpers to first modify DOM.
	// Fixes issue where hidden #acs-hidden-wp-editor is initialized again.
	acs.addAction('prepare', function(){
		html = $fields.html();
	}, 6);
	...
	$(document).ajaxComplete(function(event, xhr, settings) {
		...
		acs.doAction('remove', $fields);
		
		// reset HTML
		$fields.html( html );
		
		// action for 3rd party customization
		acs.doAction('append', $fields);
...
<?php
	...
	*/
	...
	function save_term( $term_id, $tt_id, $taxonomy ) {
		...
		if( !acs_verify_nonce('taxonomy') ) return $term_id;
	...
	*/
	...
	function delete_term( $term, $tt_id, $taxonomy, $deleted_term ) {
		...
		if( acs_isset_termmeta() ) return $term;
