...
class ACS_Form_Post {
	...
	*/
	function initialize() {
		...
		$restricted = array('acs-field-group', 'attachment');
	...
	*  add_meta_boxes
	*
	*  Adds ACS metaboxes for the given $post_type and $post.
	*
	*  @date	19/9/18
	*  @since	5.7.6
	*
	...
	function add_meta_boxes( $post_type, $post ) {
		...
		if( $field_groups ) {
			foreach( $field_groups as $field_group ) {
				...
				// vars
				$id = "acs-{$field_group['key']}";			// acs-group_123
				$title = $field_group['title'];				// Group 1
				$context = $field_group['position'];		// normal, side, acs_after_title
			...
			$this->style = acs_get_field_group_style( $field_groups[0] );
		...
		*/
		do_action('acs/add_meta_boxes', $post_type, $post, $field_groups);
	}
	...
	*/
	function edit_form_after_title() {
		...
		// render 'acs_after_title' metaboxes
		do_meta_boxes( get_current_screen(), 'acs_after_title', $post );
		
		// render dynamic field group style
		echo '<style type="text/css" id="acs-style">' . $this->style . '</style>';
	}
	
	/**
	*  render_meta_box
	*
	*  Renders the ACS metabox HTML.
	*
	*  @date	19/9/18
	*  @since	5.7.6
	*
	...
	*/
	function render_meta_box( $post, $metabox ) {
		...
		acs_render_fields( $fields, $post->ID, 'div', $field_group['instruction_placement'] );
	}
	
	/**
	*  wp_insert_post_empty_content
	*
	*  Allows WP to insert a new post without title or post_content if ACS data exists.
	*
	*  @date	16/07/2014
	*  @since	5.0.1
	*
	...
	*/
	function wp_insert_post_empty_content( $maybe_empty, $postarr ) {
		...
		// return false and allow insert if '_acs_changed' exists
		if( $maybe_empty && acs_maybe_get_POST('_acs_changed') ) {
			return false;
		}
	...
	*  Used to avoid triggering "acs/save_post" on dynamically created posts during save.
	*
	*  @type	function
	*  @date	26/06/2016
	*  @since	5.3.8
	...
	*/
	function allow_save_post( $post ) {
		...
		// restrict post types
		$restrict = array( 'auto-draft', 'revision', 'acs-field', 'acs-field-group' );
		if( in_array($post->post_type, $restrict) ) {
			$allow = false;
		}
		
		// disallow if the $_POST ID value does not match the $post->ID
		$form_post_id = (int) acs_maybe_get_POST('post_ID');
		...
		if( $post->post_type == 'revision' ) {
			...
			if( acs_maybe_get_POST('wp-preview') == 'dopreview' && $form_post_id === $post->post_parent) {
				$allow = true;
			}
		}
	...
	*/
	...
	function save_post( $post_id, $post ) {
		...
		// save
		acs_save_post( $post_id );
		
		// save revision
		if( post_type_supports($post->post_type, 'revisions') ) {
			acs_save_post_revision( $post_id );
		}
...
acs_new_instance('ACS_Form_Post');
