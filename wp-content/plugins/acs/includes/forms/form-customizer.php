...
if( ! class_exists('acs_form_customizer') ) :

class acs_form_customizer {
	...
	*/
	...
	function save_widget( $instance, $new_instance, $old_instance, $widget ) {
		...
		// append values
		$data['values'] = $new_instance['acs'];
		
		
		// append fields (name => key relationship) - used later in 'acs/get_field_reference' for customizer previews
		foreach( $data['values'] as $k => $v ) {
			
			// get field
			$field = acs_get_field( $k );
	...
	*  settings
	*
	*  This function will return an array of cutomizer settings that include ACS data
	...
	*/
	...
	function settings( $customizer ) {
		...
		foreach( $settings as $setting ) {
			...
			// bail early if no acs
			if( !is_array($value) || !isset($value['acs']) ) continue;
			
			
			// set data	
			$setting->acs = $value['acs'];
	...
	*/
	...
	function customize_preview_init( $customizer ) {
		...
		foreach( $settings as $setting ) {
			...
			// append acs_value to preview_values
		...
		// add filters
		add_filter('acs/pre_load_value', array($this, 'pre_load_value'), 10, 3);
		add_filter('acs/pre_load_reference', array($this, 'pre_load_reference'), 10, 3);
	...
	*/
	...
	function customize_save( $customizer ) {
		...
		foreach( $settings as $setting ) {
			...
			// save acs data
			acs_save_post( $data['post_id'], $data['values'] );
			
			
			// remove [acs] data from saved widget array
	...
	*  pre_update_option
	*
	*  this function will remove the [acs] data from widget insance
	*
	*  @type	function
	*  @date	22/03/2016
	*  @since	5.3.2
	...
	*/
	...
	function pre_update_option( $value, $option, $old_value ) {
		...
		foreach( $value as $i => $widget ) {
			...
			// bail ealry if no acs
			if( !isset($widget['acs']) ) continue;
...
(function($) {
	...
	// because ACS generates a unique ID on the WYSIWYG field, this string will not match causing the preview function to bail.
	...
	WidgetControl._getInputsSignature = function( inputs ) {
		...
		for( var i in signature ) {
			...
			// bail ealry if acs is found
			if( bit.indexOf('acs') !== -1 ) continue;
	...
	// either way, this function is not needed and will break ACS fields 
