...
if( ! class_exists('acs_form_nav_menu') ) :

class acs_form_nav_menu {
	...
	*/
	...
	function admin_enqueue_scripts() {
		...
		// validate screen
		if( !acs_is_screen('nav-menus') ) return;
	...
	*/
	...
	function wp_nav_menu_item_custom_fields( $item_id, $item, $depth, $args, $id = '' ) {
		...
		if( !empty($field_groups) ) {
			...
			// open
			echo '<div class="acs-menu-item-fields acs-fields -clear">';
			...
			foreach( $field_groups as $field_group ) {
				...
				// render
				acs_render_fields( $fields, $item_id, 'div', $field_group['instruction_placement'] );
			}
			...
			if( acs_is_ajax('add-menu-item') ): ?>
			<script type="text/javascript">
			(function($) {
				acs.doAction('append', $('#menu-item-settings-<?php echo $item_id; ?>') );
			})(jQuery);
			</script>
			<?php endif;
		}
	...
	*/
	...
	function update_nav_menu( $menu_id ) {
		...
		if( !acs_verify_nonce('nav_menu') ) return $menu_id;
		
			   
	    // validate and show errors
		acs_validate_save_post( true );
	...
	*/
	...
	function update_nav_menu_items( $menu_id ) {
		...
		if( empty($_POST['menu-item-acs']) ) return;
		
		
		// loop
		foreach( $_POST['menu-item-acs'] as $post_id => $values ) {
			
			acs_save_post( $post_id, $values );
	...
	*/
	...
	function wp_get_nav_menu_items( $items, $menu, $args ) {
		acs_set_data('nav_menu_id', $menu->term_id);
		return $items;
	}
	...
	function wp_edit_nav_menu_walker( $class, $menu_id = 0 ) {
		...
		acs_set_data('nav_menu_id', $menu_id);
		
		// Use custom walker class to inject "wp_nav_menu_item_custom_fields" action prioir to WP 5.4.
		if( acs_version_compare('wp', '<', '5.3.99') ) {
			acs_include('includes/walkers/class-acs-walker-nav-menu-edit.php');
			return 'ACS_Walker_Nav_Menu_Edit';
		}
	...
	*/
	...
	function acs_validate_save_post() {
		...
		foreach( $_POST['menu-item-acs'] as $post_id => $values ) {
			...
			// vars
			$prefix = 'menu-item-acs['.$post_id.']';
			
			
			// validate
			acs_validate_values( $values, $prefix );
	...
	*/
	...
	function admin_footer() {
		...
		// vars
		$nav_menu_id = acs_get_data('nav_menu_id');
...
?>
<div id="tmpl-acs-menu-settings" style="display: none;">
	<?php
	...
	if( !empty($field_groups) ) {
		...
		foreach( $field_groups as $field_group ) {
			...
			echo '<div class="acs-menu-settings -'.$field_group['style'].'">';
			
				echo '<h2>' . $field_group['title'] . '</h2>';
			
				echo '<div class="acs-fields -left -clear">';
...
(function($) {
	...
	// append html
	$('#post-body-content').append( $('#tmpl-acs-menu-settings').html() );
	...
	$(document).on('submit', '#update-nav-menu', function() {
		...
		$.each( json, function( i, pair ) {
			...
			// bail early if is 'acs[' input
			if( pair.name.indexOf('acs[') > -1 ) return;
...
acs_new_instance('acs_form_nav_menu');
