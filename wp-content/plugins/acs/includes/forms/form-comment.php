...
/*
*  ACS Comment Form Class
*
*  All the logic for adding fields to comments
*
*  @class 		acs_form_comment
*  @package		ACS
*  @subpackage	Forms
*/

if( ! class_exists('acs_form_comment') ) :
...
class acs_form_comment {
	...
	*  edit_comment
	*
	*  This function is run on the admin comment.php page and will render the ACS fields within custom metaboxes to look native
	*
	*  @type	function
	*  @date	19/10/13
	*  @since	5.0.0
	...
	function edit_comment( $comment ) {
		...
		if( !empty($field_groups) ) {
			...
			foreach( $field_groups as $field_group ) {
				...
				// vars
				$o = array(
					'id'			=> 'acs-'.$field_group['ID'],
					'key'			=> $field_group['key'],
					//'style'			=> $field_group['style'],
					'label'			=> $field_group['label_placement'],
					'edit_url'		=> '',
					'edit_title'	=> __('Edit field group', 'acs'),
				...
				?>
				<div id="acs-<?php echo $field_group['ID']; ?>" class="stuffbox">
					<h3 class="hndle"><?php echo $field_group['title']; ?></h3>
					<div class="inside">
						<?php acs_render_fields( $fields, $post_id, 'div', $field_group['instruction_placement'] ); ?>
	...
	*/
	...
	function comment_form_field_comment( $html ) {
		...
		ob_start();
			...
			echo '<div class="acs-comment-fields acs-fields -clear">';
			
			foreach( $field_groups as $field_group ) {
				
				$fields = acs_get_fields( $field_group );
				
				acs_render_fields( $fields, $post_id, 'p', $field_group['instruction_placement'] );
	...
	*/
	...
	function save_comment( $comment_id ) {
	    ...
	    if( acs_validate_save_post(true) ) {
			acs_save_post( "comment_{$comment_id}" );
		}
