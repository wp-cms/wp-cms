...
class ACS_Form_User {
	...
	*/
	...
	function admin_enqueue_scripts() {
		...
		if( !acs_is_screen(array('profile', 'user', 'user-edit')) ) {
			return;
		}
	...
	*  render
	*
	*  This function will render ACS fields for a given $post_id parameter
	*
	*  @type	function
	*  @date	7/10/13
	*  @since	5.0.0
	...
	*/
	...
	function render( $args = array() ) {
		...
		if( isset($_POST['acs']) ) {
			add_filter('acs/pre_load_value', array($this, 'filter_pre_load_value'), 10, 3);
		}
		...
		if( $args['el'] == 'div') {
			$before = '<div class="acs-user-' . $args['view'] . '-fields acs-fields -clear">';
			$after = '</div>';
		}
		...
		foreach( $field_groups as $field_group ) {
			...
			// render
			echo $before;
			acs_render_fields( $fields, $post_id, $args['el'], $field_group['instruction_placement'] );
			echo $after;
		}
				
		// actions
		add_action('acs/input/admin_footer', array($this, 'admin_footer'), 10, 1);
	}
...
<?php
	...
	*/
	...
	function save_user( $user_id ) {
		...
		// verify nonce
		if( !acs_verify_nonce('user') ) {
			return $user_id;
		}
		
	    // save
	    if( acs_validate_save_post(true) ) {
			acs_save_post( "user_$user_id" );
		}
	}
	...
	function filter_registration_errors( $errors, $sanitized_user_login, $user_email ) {
		if( !acs_validate_save_post() ) {
			$acs_errors = acs_get_validation_errors();
			foreach( $acs_errors as $acs_error ) {
				$errors->add(
					acs_idify( $acs_error['input'] ), 
					acs_punctify( sprintf( __('<strong>ERROR</strong>: %s', 'acs'), $acs_error['message'] ) )
				);
			}
		}
		return $errors;
	...
	function filter_pre_load_value( $null, $post_id, $field ) {
		$field_key = $field['key'];
		if( isset( $_POST['acs'][ $field_key ] )) {
			return $_POST['acs'][ $field_key ];
		}
		return $null;
	}
}
...
// instantiate
acs_new_instance('ACS_Form_User');
