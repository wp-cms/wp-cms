...
if( ! class_exists('ACS_Form_Gutenberg') ) :

class ACS_Form_Gutenberg {
	...
	*/
	...
	function __construct() {
		...
		add_action('acs/validate_save_post', array($this, 'acs_validate_save_post'), 999);
	}
	...
	*/
	function add_meta_boxes() {
		...
		remove_action('edit_form_after_title', array(acs_get_instance('ACS_Form_Post'), 'edit_form_after_title'));
	}
	...
	*/
	function block_editor_meta_box_hidden_fields() {
		...
		acs_get_instance('ACS_Form_Post')->edit_form_after_title();
	}
	...
	function filter_block_editor_meta_boxes( $wp_meta_boxes ) {
		...
		// Move 'acs_after_title' metaboxes into 'normal' location.
		if( isset($wp_meta_boxes[ $current_screen->id ][ 'acs_after_title' ]) ) {
	 ...
	 *
	 * Filters the `meta-box-order_{$post_type}` value by prepending "acs_after_title" data to "normal".
	 * Fixes a bug where metaboxes with position "acs_after_title" do not appear in the block editor.
	 *
	 * @date	11/7/19
	 * @since	5.8.2
	 *
	...
	function modify_user_option_meta_box_order( $locations ) {
		if( !empty($locations['acs_after_title']) ) {
			if( !empty($locations['normal']) ) {
				$locations['normal'] = $locations['acs_after_title'] . ',' . $locations['normal'];
			} else {
				$locations['normal'] = $locations['acs_after_title'];
			}
			unset($locations['acs_after_title']);
		}
		return $locations;
	}
	...
	*/
	function acs_validate_save_post() {
		...
		if( isset($_GET['meta-box-loader']) ) {
			acs_reset_validation_errors();
		}
	}
}

acs_new_instance('ACS_Form_Gutenberg');
