...
if( ! class_exists('ACS_Location_Post') ) :

class ACS_Location_Post extends ACS_Location {
		...
		if( $groups ) {
			foreach( $groups as $label => $posts ) {
				...
				foreach( $posts as $post ) {
					$choices[ $label ][ $post->ID ] = acs_get_post_title( $post );
				}
			}
		}
		return $choices;
...
// initialize
acs_register_location_type( 'ACS_Location_Post' );
