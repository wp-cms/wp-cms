...
if( ! class_exists('ACS_Location_Post_Taxonomy') ) :

class ACS_Location_Post_Taxonomy extends ACS_Location {
	...
	public function initialize() {
		...
		$this->label = __( "Post Taxonomy", 'acs' );
		$this->category = 'post';
	...
	public function match( $rule, $screen, $field_group ) {
		...
		if( isset($screen['post_terms']) ) {
			$post_terms = acs_maybe_get( $screen['post_terms'], $term->taxonomy, array() );
		} else {
	...
	public function get_object_subtype( $rule ) {
		if( $rule['operator'] === '==' ) {
			$term = acs_decode_term( $rule['value'] );
			if( $term ) {
...
// initialize
acs_register_location_type( 'ACS_Location_Post_Taxonomy' );
