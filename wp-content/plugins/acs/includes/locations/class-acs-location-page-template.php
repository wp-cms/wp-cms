...
if( ! class_exists('ACS_Location_Page_Template') ) :

class ACS_Location_Page_Template extends ACS_Location {
	...
	public function initialize() {
		...
		$this->label = __( "Page Template", 'acs' );
		$this->category = 'page';
	...
	public function match( $rule, $screen, $field_group ) {
		...
		return acs_get_location_type( 'post_template' )->match( $rule, $screen, $field_group );
	}
	...
	public function get_values( $rule ) {
		$post_templates = acs_get_post_templates();
		return array_merge(
			array(
				'default' => apply_filters( 'default_page_template_title',  __('Default Template', 'acs') )
			),
			$post_templates[ 'page' ]
		);
	}
...
// Register.
acs_register_location_type( 'ACS_Location_Page_Template' );
