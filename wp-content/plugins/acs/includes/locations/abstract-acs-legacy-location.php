...
if( ! class_exists('ACS_Legacy_Location') ) :

abstract class ACS_Legacy_Location {
	...
	public function __construct() {
		...
		if( method_exists($this, 'rule_match') ) {
			add_filter( "acs/location/rule_match/{$this->name}", array( $this, 'rule_match' ), 5, 3 );
		}
		if( method_exists($this, 'rule_operators') ) {
			add_filter( "acs/location/rule_operators/{$this->name}", array( $this, 'rule_operators' ), 5, 2 );
		}
		if( method_exists($this, 'rule_values') ) {
			add_filter( "acs/location/rule_values/{$this->name}", array( $this, 'rule_values' ), 5, 2 );
		}
	}
