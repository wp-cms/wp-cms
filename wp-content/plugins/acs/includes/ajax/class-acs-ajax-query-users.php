...
if( ! class_exists('ACS_Ajax_Query_Users') ) :

class ACS_Ajax_Query_Users extends ACS_Ajax_Query {
	
	/** @var string The AJAX action name. */
	var $action = 'acs/ajax/query_users';
	...
	function init_request( $request ) {
		 ...
		 */
		do_action( "acs/ajax/query_users/init", $request, $this );
	}
	...
	function get_args( $request ) {
		 ...
		 * @param	ACS_Ajax_Query $query The query object.
		 */
		return apply_filters( "acs/ajax/query_users/args", $args, $request, $this );
	}
	...
	function get_results( $args ) {
		...
		if( !empty( $args['role__in']) ) {
			$roles = acs_get_user_role_labels( $args['role__in'] );
		} else {
			$roles = acs_get_user_role_labels();
		}
		 ...
		 * @param	ACS_Ajax_Query $query The query object.
		 */
		return apply_filters( "acs/ajax/query_users/results", $results, $args, $this );
	}
	...
	function get_result( $user ) {
		 ...
		 * @param	ACS_User $user The user object.
		 * @param	ACS_Ajax_Query $query The query object.
		 */
		return apply_filters( "acs/ajax/query_users/result", $item, $user, $this );
	}
	...
	function filter_search_columns( $columns, $search, $WP_User_Query ) {
		 ...
		 * @param	ACS_Ajax_Query $query The query object.
		 */
		return apply_filters( "acs/ajax/query_users/search_columns", $columns, $search, $WP_User_Query, $this );
	}
}

acs_new_instance('ACS_Ajax_Query_Users');
