...
if( ! class_exists('ACS_Ajax_Query') ) :

class ACS_Ajax_Query extends ACS_Ajax {
	...
	/** @var array The ACS field related to this query. */
	var $field = false;
	...
	function init_request( $request ) {
		...
		if( isset($request['field_key']) ) {
			$this->field = acs_get_field( $request['field_key'] );
		}
		...
		if( isset($request['search']) && acs_not_empty($request['search']) ) {
