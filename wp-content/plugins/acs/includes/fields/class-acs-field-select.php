...
if( ! class_exists('acs_field_select') ) :

class acs_field_select extends acs_field {
	...
	*/
	...
	function initialize() {
		...
		// ajax
		add_action('wp_ajax_acs/fields/select/query',				array($this, 'ajax_query'));
		add_action('wp_ajax_nopriv_acs/fields/select/query',		array($this, 'ajax_query'));
	...
	*/
	...
	function input_admin_enqueue_scripts() {
		...
		// bail ealry if no enqueue
	   	if( !acs_get_setting('enqueue_select2') ) return;
		...
		$major = acs_get_setting('select2_version');
		$version = '';
		$script = '';
		$style = '';
		...
		if( $major == 4 ) {
			...
			$version = '4.0';
			$script = acs_get_url("assets/inc/select2/4/select2.full{$min}.js");
			$style = acs_get_url("assets/inc/select2/4/select2{$min}.css");
		
		// v3
		} else {
			
			$version = '3.5.2';
			$script = acs_get_url("assets/inc/select2/3/select2{$min}.js");
			$style = acs_get_url("assets/inc/select2/3/select2.css");
		...
		acs_localize_data(array(
		   	'select2L10n'	=> array(
				'matches_1'				=> _x('One result is available, press enter to select it.',	'Select2 JS matches_1',	'acs'),
				'matches_n'				=> _x('%d results are available, use up and down arrow keys to navigate.',	'Select2 JS matches_n',	'acs'),
				'matches_0'				=> _x('No matches found',	'Select2 JS matches_0',	'acs'),
				'input_too_short_1'		=> _x('Please enter 1 or more characters', 'Select2 JS input_too_short_1', 'acs' ),
				'input_too_short_n'		=> _x('Please enter %d or more characters', 'Select2 JS input_too_short_n', 'acs' ),
				'input_too_long_1'		=> _x('Please delete 1 character', 'Select2 JS input_too_long_1', 'acs' ),
				'input_too_long_n'		=> _x('Please delete %d characters', 'Select2 JS input_too_long_n', 'acs' ),
				'selection_too_long_1'	=> _x('You can only select 1 item', 'Select2 JS selection_too_long_1', 'acs' ),
				'selection_too_long_n'	=> _x('You can only select %d items', 'Select2 JS selection_too_long_n', 'acs' ),
				'load_more'				=> _x('Loading more results&hellip;', 'Select2 JS load_more', 'acs' ),
				'searching'				=> _x('Searching&hellip;', 'Select2 JS searching', 'acs' ),
				'load_fail'           	=> _x('Loading failed', 'Select2 JS load_fail', 'acs' ),
			)
	   	));
	}
	...
	*/
	...
	function get_ajax_query( $options = array() ) {
		...
		// get choices
		$choices = acs_get_array($field['choices']);
	...
	*/
	...
	function render_field( $field ) {
		...
		// convert
		$value = acs_get_array($field['value']);
		$choices = acs_get_array($field['choices']);
		
		
		// placeholder
		if( empty($field['placeholder']) ) {
			$field['placeholder'] = _x('Select', 'verb', 'acs');
		}
	...
	*/
	...
	function render_field_settings( $field ) {
		...
		acs_render_field_setting( $field, array(
			...
			'instructions'	=> __('Specify the value returned','acs'),
			'type'			=> 'select',
			'name'			=> 'return_format',
			'choices'		=> array(
				'value'			=> __('Value','acs'),
	...
	*/
	function load_value( $value, $post_id, $field ) {
		...
		if( $field['multiple'] ) {
			...
			return acs_array( $value );
		}

		// Otherwise, return a single value.
		return acs_unarray( $value );
	}
	...
	*/
	...
	function translate_field( $field ) {
		...
		// translate
		$field['choices'] = acs_translate( $field['choices'] );
	...
	*/
	...
	function format_value_single( $value, $post_id, $field ) {
		...
		if( acs_is_empty($value) ) return $value;
		
		
		// vars
		$label = acs_maybe_get($field['choices'], $value, $value);
...
// initialize
acs_register_field_type( 'acs_field_select' );
