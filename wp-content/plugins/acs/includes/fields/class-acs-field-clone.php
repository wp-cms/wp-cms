...
if( ! class_exists('acs_field_clone') ) :

class acs_field_clone extends acs_field {
	...
	*/
	...
	function initialize() {
		...
		// register filter
		acs_enable_filter('clone');
		
		
		// ajax
		add_action('wp_ajax_acs/fields/clone/query', array($this, 'ajax_query'));
		
		
		// filters
		add_filter('acs/get_fields', 		array($this, 'acs_get_fields'), 5, 2);
		add_filter('acs/prepare_field',		array($this, 'acs_prepare_field'), 10, 1);
		add_filter('acs/clone_field',		array($this, 'acs_clone_field'), 10, 2);
	...
	*  is_enabled
	*
	*  This function will return true if acs_local functionality is enabled
	*
	*  @type	function
	*  @date	14/07/2016
	*  @since	5.4.0
	...
	*/
	...
	function is_enabled() {
		...
		return acs_is_filter_enabled('clone');
	...
	*  acs_get_fields
	*
	*  This function will hook into the 'acs/get_fields' filter and inject/replace seamless clones fields
	*
	*  @type	function
	*  @date	17/06/2016
	*  @since	5.3.8
	...
	*/
	...
	function acs_get_fields( $fields, $parent ) {
	...
	*/
	...
	function get_cloned_fields( $field ) {
		...
		foreach( $field['clone'] as $selector ) {
			...
			if( acs_is_field_group_key($selector) ) {
				
				$field_group = acs_get_field_group( $selector );
				if( !$field_group ) {
					continue;
				}
				
				$field_group_fields = acs_get_fields( $field_group );
			...
			// Field selector.
			} elseif( acs_is_field_key($selector) ) {
				$fields[] = acs_get_field( $selector );
			}
		...
		// loop
		// run acs_clone_field() on each cloned field to modify name, key, etc
		foreach( array_keys($fields) as $i ) {
			
			$fields[ $i ] = acs_clone_field( $fields[ $i ], $field );
	...
	*  Important to run the acs_clone_field function on sub fields to pass on settings such as 'parent_layout'
	*
	*  @type	function
	*  @date	28/06/2016
	*  @since	5.3.8
	...
	*/
	...
	function acs_clone_field( $field, $clone_field ) {
		...
		if( $clone_field['display'] == 'seamless' ) {
			...
			// - the original key will later be restored by acs/prepare_field allowing conditional logic JS to work
		...
		if( $field['type'] == 'clone' ) {
			...
			$field = $this->acs_clone_clone_field( $field, $clone_field );
	...
	*/
	...
	function acs_clone_clone_field( $field, $clone_field ) {
		
		// modify the $clone_field name
		// This seems odd, however, the $clone_field is later passed into the acs_clone_field() function
		// Do not delete! 
		...
		foreach( $field['sub_fields'] as &$sub_field ) {
			...
			// clone
			$sub_field = acs_clone_field( $sub_field, $clone_field );
	...
	*/
	...
	function prepare_field_for_db( $field ) {
		...
		//acs_log('== prepare_field_for_db ==');
		//acs_log('- clone name:', $field['name']);
		//acs_log('- clone _name:', $field['_name']);
	...
	*/
	...
	function load_value( $value, $post_id, $field ) {
		...
		foreach( $field['sub_fields'] as $sub_field ) {
			...
			// add value
			$value[ $sub_field['key'] ] = acs_get_value( $post_id, $sub_field );
	...
	*/
	...
	function format_value( $value, $post_id, $field ) {
		...
		foreach( $field['sub_fields'] as $sub_field ) {
			...
			// extract value
			$sub_value = acs_extract_var( $value, $sub_field['key'] );
	...
	*/
	...
	function update_value( $value, $post_id, $field ) {
		...
		// bail early if no value
		if( !acs_is_array($value) ) return null;
		...
		foreach( $field['sub_fields'] as $sub_field ) {
			...
			$sub_field = $this->acs_prepare_field( $sub_field );
			
			
			// update value
			acs_update_value( $v, $post_id, $sub_field );
	...
	*/
	...
	function render_field_block( $field ) {
		...
		// html
		echo '<div class="acs-clone-fields acs-fields -'.$label_placement.' -border">';
			
		foreach( $field['sub_fields'] as $sub_field ) {
			
			acs_render_field_wrap( $sub_field );
...
<table class="acs-table">
	...
	<tbody>
		...
		foreach( $field['sub_fields'] as $sub_field ) {
			...
			acs_render_field_wrap( $sub_field, 'td' );
...
<?php
	...
	*/
	...
	function render_field_settings( $field ) {
		...
		acs_render_field_setting( $field, array(
			'label'			=> __('Fields', 'acs'),
			'instructions'	=> __('Select one or more fields you wish to clone','acs'),
			'type'			=> 'select',
			'name'			=> 'clone',
			'multiple' 		=> 1,
			'allow_null' 	=> 1,
			...
			'ui'			=> 1,
			'ajax'			=> 1,
			'ajax_action'	=> 'acs/fields/clone/query',
			'placeholder'	=> '',
		));
		...
		// display
		acs_render_field_setting( $field, array(
			'label'			=> __('Display','acs'),
			'instructions'	=> __('Specify the style used to render the clone field', 'acs'),
			'type'			=> 'select',
			'name'			=> 'display',
			'class'			=> 'setting-display',
			'choices'		=> array(
				'group'			=> __('Group (displays selected fields in a group within this field)','acs'),
				'seamless'		=> __('Seamless (replaces this field with selected fields)','acs'),
			),
		));
		...
		acs_render_field_setting( $field, array(
			'label'			=> __('Layout','acs'),
			'instructions'	=> __('Specify the style used to render the selected fields', 'acs'),
			'type'			=> 'radio',
			'name'			=> 'layout',
			'layout'		=> 'horizontal',
			'choices'		=> array(
		...
		// prefix_label
		$instructions = __('Labels will be displayed as %s', 'acs');
		$instructions = sprintf($instructions, '<code class="prefix-label-code-1"></code>');
		acs_render_field_setting( $field, array(
			'label'			=> __('Prefix Field Labels','acs'),
		...
		// prefix_name
		$instructions = __('Values will be saved as %s', 'acs');
		$instructions = sprintf($instructions, '<code class="prefix-name-code-1"></code>');
		acs_render_field_setting( $field, array(
			'label'			=> __('Prefix Field Names','acs'),
			'message'	=> $instructions,
			//'instructions_placement'	=> 'field',
			'name'			=> 'prefix_name',
	...
	*/
	...
	function get_clone_setting_choice( $selector = '' ) {
		...
		// field
		if( acs_is_field_key($selector) ) {
			
			return $this->get_clone_setting_field_choice( acs_get_field($selector) );
		...
		if( acs_is_field_group_key($selector) ) {
			...
			return $this->get_clone_setting_group_choice( acs_get_field_group($selector) );
	...
	*/
	...
	function get_clone_setting_field_choice( $field ) {
		...
		if( !$field ) return __('Unknown field', 'acs');
		
		
		// title
		$title = $field['label'] ? $field['label'] : __('(no title)', 'acs');
		...
		$ancestors = isset($field['ancestors']) ? $field['ancestors'] : count(acs_get_field_ancestors($field));
	...
	*/
	...
	function get_clone_setting_group_choice( $field_group ) {
		...
		if( !$field_group ) return __('Unknown field group', 'acs');
		
		
		// return
		return sprintf( __('All fields from %s field group', 'acs'), $field_group['title'] );
	...
	*/
	...
	function ajax_query() {
		...
		foreach( array_keys($field_groups) as $j ) {
			...
			$field_group = acs_extract_var($field_groups, $j);
	...
	*/
	...
	function validate_value( $valid, $value, $field, $input ){
		...
		foreach( array_keys($field['sub_fields']) as $i ) {
			...
			// validate
			acs_validate_value( $value[ $k ], $sub_field, "{$input}[{$k}]" );
...
// initialize
acs_register_field_type( 'acs_field_clone' );
