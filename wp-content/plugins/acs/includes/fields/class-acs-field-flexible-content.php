...
if( ! class_exists('acs_field_flexible_content') ) :

class acs_field_flexible_content extends acs_field {
	...
	*/
	...
	function initialize() {
		...
		$this->label = __("Flexible Content",'acs');
		$this->category = 'layout';
		$this->defaults = array(
			'layouts'		=> array(),
			'min'			=> '',
			'max'			=> '',
			'button_label'	=> __("Add Row",'acs'),
		);
		
		
		// ajax
		$this->add_action('wp_ajax_acs/fields/flexible_content/layout_title',			array($this, 'ajax_layout_title'));
		$this->add_action('wp_ajax_nopriv_acs/fields/flexible_content/layout_title',	array($this, 'ajax_layout_title'));
		
		
		// filters
		$this->add_filter('acs/prepare_field_for_export',	array($this, 'prepare_any_field_for_export'));
		$this->add_filter('acs/clone_field', 				array($this, 'clone_any_field'), 10, 2);
		$this->add_filter('acs/validate_field',					array($this, 'validate_any_field'));
		
		
		// field filters
		$this->add_field_filter('acs/get_sub_field', 			array($this, 'get_sub_field'), 10, 3);
	...
	*/
	...
	function input_admin_enqueue_scripts() {
		...
		acs_localize_text(array(
			...
			'layouts'													=> __('layouts', 'acs'),
			
			// min / max
			'This field requires at least {min} {label} {identifier}'	=> __('This field requires at least {min} {label} {identifier}', 'acs'),
			'This field has a limit of {max} {label} {identifier}'		=> __('This field has a limit of {max} {label} {identifier}', 'acs'),
			
			// popup badge
			'{available} {label} {identifier} available (max {max})'	=> __('{available} {label} {identifier} available (max {max})', 'acs'),
			'{required} {label} {identifier} required (min {min})'		=> __('{required} {label} {identifier} required (min {min})', 'acs'),
			
			// field settings
			'Flexible Content requires at least 1 layout'				=> __('Flexible Content requires at least 1 layout', 'acs')
	   	));
	}
	...
	*/
	...
	function load_field( $field ) {
		...
		foreach( array_keys($field['layouts']) as $i ) {
			...
			// extract layout
			$layout = acs_extract_var( $field['layouts'], $i );
			...
			if( !empty($sub_fields) ) {
				...
				foreach( array_keys($sub_fields) as $k ) {
					...
					if( $sub_fields[ $k ]['parent_layout'] == $layout['key'] ) {
						...
						$layout['sub_fields'][] = acs_extract_var( $sub_fields, $k );
		...
		if( $field['layouts'] ) {
			foreach( $field['layouts'] as $layout ) {
				...
				if( $layout['sub_fields'] ) {
					$sub_field = acs_search_fields( $id, $layout['sub_fields'] );
					if( $sub_field ) {
						break;
					}
				}
	...
	*/
	...
	function render_field( $field ) {
		...
		// no value message
		$no_value_message = __('Click the "%s" button below to start creating your layout','acs');
		$no_value_message = apply_filters('acs/fields/flexible_content/no_value_message', $no_value_message, $field);

?>
<div <?php acs_esc_attr_e( $div ); ?>>
	
	<?php acs_hidden_input(array( 'name' => $field['name'] )); ?>
	...
	<div class="clones">
		<?php foreach( $layouts as $layout ): ?>
			<?php $this->render_layout( $field, $layout, 'acscloneindex', array() ); ?>
		<?php endforeach; ?>
	</div>
	...
	<div class="values">
		<?php if( !empty($field['value']) ): 
			...
			foreach( $field['value'] as $i => $value ):
				...
				// validate
				if( empty($layouts[ $value['acs_fc_layout'] ]) ) continue;
				
				
				// render
				$this->render_layout( $field, $layouts[ $value['acs_fc_layout'] ], $i, $value );
	...
	<div class="acs-actions">
		<a class="acs-button button button-primary" href="#" data-name="add-layout"><?php echo $field['button_label']; ?></a>
	</div>
	...
	<script type="text-html" class="tmpl-popup"><?php 
		?><ul><?php foreach( $layouts as $layout ): 
			...
			?><li><a <?php acs_esc_attr_e( $atts ); ?>><?php echo $layout['label']; ?></a></li><?php 
...
<div <?php echo acs_esc_attr($div); ?>>
	...
	<?php acs_hidden_input(array( 'name' => $prefix.'[acs_fc_layout]', 'value' => $layout['name'] )); ?>
	
	<div class="acs-fc-layout-handle" title="<?php _e('Drag to reorder','acs'); ?>" data-name="collapse-layout"><?php echo $title; ?></div>
	
	<div class="acs-fc-layout-controls">
		<a class="acs-icon -plus small light acs-js-tooltip" href="#" data-name="add-layout" title="<?php _e('Add layout','acs'); ?>"></a>
		<a class="acs-icon -duplicate small light acs-js-tooltip" href="#" data-name="duplicate-layout" title="<?php _e('Duplicate layout','acs'); ?>"></a>
		<a class="acs-icon -minus small light acs-js-tooltip" href="#" data-name="remove-layout" title="<?php _e('Remove layout','acs'); ?>"></a>
		<a class="acs-icon -collapse small -clear acs-js-tooltip" href="#" data-name="collapse-layout" title="<?php _e('Click to toggle','acs'); ?>"></a>
	</div>
...
<?php if( !empty($sub_fields) ): ?>
	...
	<table class="acs-table">
		...
		<tbody>
			<tr class="acs-row">
	<?php else: ?>
	<div class="acs-fields <?php if($layout['display'] == 'row'): ?>-left<?php endif; ?>">
	<?php endif; ?>
...
?><tr class="acs-field acs-field-setting-fc_layout" data-name="fc_layout" data-setting="flexible_content" data-id="<?php echo $layout['key']; ?>">
	<td class="acs-label">
		<label><?php _e("Layout",'acs'); ?></label>
		<ul class="acs-bl acs-fl-actions">
			<li><a class="reorder-layout" href="#" title="<?php _e("Reorder Layout",'acs'); ?>"><?php _e("Reorder",'acs'); ?></a></li>
			<li><a class="delete-layout" href="#" title="<?php _e("Delete Layout",'acs'); ?>"><?php _e("Delete",'acs'); ?></a></li>
			<li><a class="duplicate-layout" href="#" title="<?php _e("Duplicate Layout",'acs'); ?>"><?php _e("Duplicate",'acs'); ?></a></li>
			<li><a class="add-layout" href="#" title="<?php _e("Add New Layout",'acs'); ?>"><?php _e("Add New",'acs'); ?></a></li>
		</ul>
	</td>
	<td class="acs-input">
		<?php 
		...
		acs_hidden_input(array(
			'id'		=> acs_idify( $layout_prefix . '[key]' ),
		...
		?>
		<ul class="acs-fc-meta acs-bl">
			<li class="acs-fc-meta-label">
				<?php 
				...
				acs_render_field(array(
					...
					'value'		=> $layout['label'],
					'prepend'	=> __('Label','acs')
				));
				
				?>
			</li>
			<li class="acs-fc-meta-name">
				<?php 
			...
			<li class="acs-fc-meta-display">
				<div class="acs-input-prepend"><?php _e('Layout','acs'); ?></div>
				<div class="acs-input-wrap">
					<?php 
...
<?php
	...
	*/
	...
	function validate_value( $valid, $value, $field, $input ){
		...
		if( $min && $count < $min ) {
			...
			// vars
			$error = __('This field requires at least {min} {label} {identifier}', 'acs');
		...
		if( $count ) {
			...
			foreach( $value as $i => $row ) {	
				...
				foreach( $layouts[ $l ]['sub_fields'] as $sub_field ) {
					...
					// validate
					acs_validate_value( $value[ $i ][ $k ], $sub_field, "{$input}[{$i}][{$k}]" );
				}
				// end loop sub fields
	...
	function delete_row( $i, $field, $post_id ) {
		...
		// vars
		$value = acs_get_metadata( $post_id, $field['name'] );
	...
	function update_row( $row, $i, $field, $post_id ) {
		...
		if( !is_array($row) || !isset($row['acs_fc_layout']) ) return false;
		
		
		// get layout
		$layout = $this->get_layout($row['acs_fc_layout'], $field);
	...
	*/
	...
	function update_value( $value, $post_id, $field ) {
		...
		// vars
		$new_value = array();
		$old_value = acs_get_metadata( $post_id, $field['name'] );
		...
		if( !empty($value) ) { $i = -1;
			...
			foreach( $value as $row ) {	$i++;
				...
				// bail early if no layout reference
				if( !is_array($row) || !isset($row['acs_fc_layout']) ) continue;
				
				
				// delete old row if layout has changed
				if( isset($old_value[ $i ]) && $old_value[ $i ] !== $row['acs_fc_layout'] ) {
	...
	function duplicate_field( $field ) {
		...
		if( !empty($field['layouts']) ) {
			...
			foreach( $field['layouts'] as $layout ) {
				...
				// extract sub fields
				$extra = acs_extract_var( $layout, 'sub_fields' );
	...
	*/
	...
	function ajax_layout_title() {
		...
		// options
   		$options = acs_parse_args( $_POST, array(
			'post_id'		=> 0,
			'i'				=> 0,
			'field_key'		=> '',
			'nonce'			=> '',
	...
	function get_layout_title( $field, $layout, $i, $value ) {
		...
		// filters
		$title = apply_filters('acs/fields/flexible_content/layout_title', 							$title, $field, $layout, $i);
		$title = apply_filters('acs/fields/flexible_content/layout_title/name='.$field['_name'],	$title, $field, $layout, $i);
		$title = apply_filters('acs/fields/flexible_content/layout_title/key='.$field['key'],		$title, $field, $layout, $i);
		...
		$title = '<span class="acs-fc-layout-order">' . $order . '</span> ' . $title;
	...
	function prepare_field_for_export( $field ) {
		...
		if( !empty($field['layouts']) ) {
			...
			foreach( $field['layouts'] as &$layout ) {
				...
				$layout['sub_fields'] = acs_prepare_fields_for_export( $layout['sub_fields'] );
	...
	*/
	...
	function prepare_field_for_import( $field ) {
		...
		foreach( $field['layouts'] as &$layout ) {
			...
			// Extract sub fields.
			$sub_fields = acs_extract_var( $layout, 'sub_fields' );
	...
	function translate_field( $field ) {
		...
		if( !empty($field['layouts']) ) {
			...
			foreach( $field['layouts'] as &$layout ) {
				...
				$layout['label'] = acs_translate($layout['label']);
...
// initialize
acs_register_field_type( 'acs_field_flexible_content' );
