...
if( ! class_exists('acs_field_taxonomy') ) :

class acs_field_taxonomy extends acs_field {
	...
	*/
	...
	function initialize() {
		...
		acs_add_filter_variations( 'acs/fields/taxonomy/query', array('name', 'key'), 1 );
		acs_add_filter_variations( 'acs/fields/taxonomy/result', array('name', 'key'), 2 );
		
		
		// ajax
		add_action('wp_ajax_acs/fields/taxonomy/query',			array($this, 'ajax_query'));
		add_action('wp_ajax_nopriv_acs/fields/taxonomy/query',	array($this, 'ajax_query'));
		add_action('wp_ajax_acs/fields/taxonomy/add_term',		array($this, 'ajax_add_term'));
		
		
		// actions
		add_action('acs/save_post', array($this, 'save_post'), 15, 1);
	...
	*/
	...
	function get_ajax_query( $options = array() ) {
		...
		// filters
		$args = apply_filters('acs/fields/taxonomy/query', $args, $field, $options['post_id']);
		
		
		// get terms
		$terms = acs_get_terms( $args );
		...
		if( $is_hierarchical ) {
			...
			// update vars
			$limit = acs_maybe_get( $args, 'number', $limit );
			$offset = acs_maybe_get( $args, 'offset', $offset );
			
			
			// get parent
			$parent = acs_maybe_get( $args, 'parent', 0 );
			$parent = acs_maybe_get( $args, 'child_of', $parent );
	...
	function get_term_title( $term, $field, $post_id = 0 ) {
		 ...
		 */
		 return apply_filters('acs/fields/taxonomy/result', $title, $term, $field, $post_id);
	}
	...
	*/
	...
	function load_value( $value, $post_id, $field ) {
		...
		// get valid terms
		$value = acs_get_valid_terms($value, $field['taxonomy']);
...
<div <?php acs_esc_attr_e($div); ?>>
	...
	<div class="acs-actions -hover">
		<a href="#" class="acs-icon -plus acs-js-tooltip small" data-name="add" title="<?php echo esc_attr($taxonomy->labels->add_new_item); ?>"></a>
	</div>
	<?php endif;
	...
	*/
	...
	function render_field_checkbox( $field ) {
		...
		// include walker
		acs_include('includes/walkers/class-acs-walker-taxonomy-field.php');
		
		
		// vars
		$args = array(
			'taxonomy'     		=> $field['taxonomy'],
			'show_option_none'	=> sprintf( _x('No %s', 'No terms', 'acs'), strtolower($taxonomy_obj->labels->name) ),
			'hide_empty'   		=> false,
			'style'        		=> 'none',
			'walker'       		=> new ACS_Taxonomy_Field_Walker( $field ),
		);
		
		
		// filter for 3rd party customization
		$args = apply_filters('acs/fields/taxonomy/wp_list_categories', $args, $field);
		$args = apply_filters('acs/fields/taxonomy/wp_list_categories/name=' . $field['_name'], $args, $field);
		$args = apply_filters('acs/fields/taxonomy/wp_list_categories/key=' . $field['key'], $args, $field);
		
		?>
<div class="categorychecklist-holder">
	<ul class="acs-checkbox-list acs-bl">
	...
	*/
	...
	function render_field_settings( $field ) {
		...
		acs_render_field_setting( $field, array(
			'label'			=> __('Taxonomy','acs'),
			'instructions'	=> __('Select the taxonomy to be displayed','acs'),
			'type'			=> 'select',
			'name'			=> 'taxonomy',
		...
		acs_render_field_setting( $field, array(
			...
			'optgroup'		=> true,
			'choices'		=> array(
				__("Multiple Values",'acs') => array(
					'checkbox' => __('Checkbox', 'acs'),
					'multi_select' => __('Multi Select', 'acs')
				),
				__("Single Value",'acs') => array(
		...
		// add_term
		acs_render_field_setting( $field, array(
			'label'			=> __('Create Terms','acs'),
			'instructions'	=> __('Allow new terms to be created whilst editing','acs'),
			'name'			=> 'add_term',
			'type'			=> 'true_false',
			'ui'			=> 1,
		));
		...
		// save_terms
		acs_render_field_setting( $field, array(
			'label'			=> __('Save Terms','acs'),
			'instructions'	=> __('Connect selected terms to the post','acs'),
			'name'			=> 'save_terms',
			'type'			=> 'true_false',
			'ui'			=> 1,
		));
		...
		// load_terms
		acs_render_field_setting( $field, array(
			'label'			=> __('Load Terms','acs'),
			'instructions'	=> __('Load value from posts terms','acs'),
			'name'			=> 'load_terms',
			'type'			=> 'true_false',
			'ui'			=> 1,
		));
	...
	function ajax_add_term() {
		...
		if( !current_user_can( $taxonomy_obj->cap->manage_terms) ) {
			wp_send_json_error(array(
				'error'	=> sprintf( __('User unable to add new %s', 'acs'), $taxonomy_label )
			));
		}
		...
		if( $args['term_name'] ) {
			...
			if( term_exists($args['term_name'], $field['taxonomy'], $args['term_parent']) ) {
				wp_send_json_error(array(
					'error'	=> sprintf( __('%s already exists', 'acs'), $taxonomy_label )
				));
			}
			...
			wp_send_json_success(array(
				'message'		=> sprintf( __('%s added', 'acs'), $taxonomy_label ),
		...
		?><p class="acs-submit">
			<button class="acs-submit-button button button-primary" type="submit"><?php _e("Add", 'acs'); ?></button>
		</p>
		</form><?php
...
// initialize
acs_register_field_type( 'acs_field_taxonomy' );
