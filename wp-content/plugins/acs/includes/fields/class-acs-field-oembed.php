...
if( ! class_exists('acs_field_oembed') ) :

class acs_field_oembed extends acs_field {
	...
	*/
	...
	function initialize() {
		...
		// extra
		add_action('wp_ajax_acs/fields/oembed/search',			array($this, 'ajax_query'));
		add_action('wp_ajax_nopriv_acs/fields/oembed/search',	array($this, 'ajax_query'));
	...
	*/
	...
	function get_ajax_query( $args = array() ) {
   		...
   		// defaults
   		$args = acs_parse_args($args, array(
			's'				=> '',
			'field_key'		=> '',
		));
	...
	*/
	...
	function render_field( $field ) {
		...
		// <strong><?php _e("Error.", 'acs'); </strong> _e("No embed found for the given URL.", 'acs');
...
<div <?php acs_esc_attr_e($atts) ?>>
	...
	<?php acs_hidden_input(array( 'class' => 'input-value', 'name' => $field['name'], 'value' => $field['value'] )); ?>
	
	<div class="title">
		<?php acs_text_input(array( 'class' => 'input-search', 'value' => $field['value'], 'placeholder' => __("Enter URL", 'acs'), 'autocomplete' => 'off'  )); ?>
		<div class="acs-actions -hover">
			<a data-name="clear-button" href="#" class="acs-icon -cancel grey"></a>
		</div>
	</div>
	...
	<div class="canvas">
		...
		<i class="acs-icon -picture hide-if-value"></i>
	</div>
...
// initialize
acs_register_field_type( 'acs_field_oembed' );
