...
if ( ! class_exists( 'acs_field_radio' ) ) {

	class acs_field_radio extends acs_field {
		...
		*/
		...
		function initialize() {
			...
			// vars
			$this->name     = 'radio';
			$this->label    = __( 'Radio Button', 'acs' );
		...
		*/
		...
		function render_field( $field ) {
			...
			// append to class
			$ul['class'] .= ' ' . ( 'horizontal' === $field['layout'] ? 'acs-hl' : 'acs-bl' );
			...
			if ( $field['other_choice'] ) {
				...
				// append other choice
				$field['choices']['other'] .= '</label> <input type="text" ' . acs_esc_attr( $input ) . ' /><label>';
			...
			// hiden input
			$e .= acs_get_hidden_input( array( 'name' => $field['name'] ) );

			// open
			$e .= '<ul ' . acs_esc_attr( $ul ) . '>';
			...
			foreach ( $field['choices'] as $value => $label ) {
				...
				// deisabled
				if ( isset( $field['disabled'] ) && acs_in_array( $value, $field['disabled'] ) ) {
				...
				// append
				$e .= '<li><label' . $class . '><input ' . acs_esc_attr( $atts ) . '/>' . $label . '</label></li>';
		...
		*/
		...
		function render_field_settings( $field ) {
			...
			acs_render_field_setting(
				$field,
				array(
					'label'        => __( 'Save Other', 'acs' ),
					'instructions' => '',
					'name'         => 'save_other_choice',
					'type'         => 'true_false',
					'ui'           => 1,
					'message'      => __( "Save 'other' values to the field's choices", 'acs' ),
					'conditions'   => array(
						'field'    => 'other_choice',
						'operator' => '==',
						'value'    => 1,
		...
		function update_value( $value, $post_id, $field ) {
			...
			if ( $field['save_other_choice'] ) {
				...
				if ( ! isset( $field['choices'][ $value ] ) ) {
					...
					$field    = acs_get_field( $selector, true );
	...
	// initialize
	acs_register_field_type( 'acs_field_radio' );
