...
if( ! class_exists('acs_field_page_link') ) :

class acs_field_page_link extends acs_field {
	...
	*/
	...
	function initialize() {
		...
		// extra
		add_action('wp_ajax_acs/fields/page_link/query',			array($this, 'ajax_query'));
		add_action('wp_ajax_nopriv_acs/fields/page_link/query',		array($this, 'ajax_query'));
	...
	*/
	...
	function ajax_query() {
		...
		if( !empty($field['taxonomy']) ) {
			...
			// decode terms
			$taxonomies = acs_decode_taxonomy_terms( $field['taxonomy'] );
		...
		// filters
		$args = apply_filters('acs/fields/page_link/query', $args, $field, $options['post_id']);
		$args = apply_filters('acs/fields/page_link/query/name=' . $field['name'], $args, $field, $options['post_id'] );
		$args = apply_filters('acs/fields/page_link/query/key=' . $field['key'], $args, $field, $options['post_id'] );
	...
	*/
	...
	function get_post_title( $post, $field, $post_id = 0, $is_search = 0 ) {
		...
		// filters
		$title = apply_filters('acs/fields/page_link/result', $title, $post, $field, $post_id);
		$title = apply_filters('acs/fields/page_link/result/name=' . $field['_name'], $title, $post, $field, $post_id);
		$title = apply_filters('acs/fields/page_link/result/key=' . $field['key'], $title, $post, $field, $post_id);
	...
	*/
	...
	function render_field_settings( $field ) {
		...
		// allow_archives
		acs_render_field_setting( $field, array(
			'label'			=> __('Allow Archives URLs','acs'),
			'instructions'	=> '',
			'name'			=> 'allow_archives',
			'type'			=> 'true_false',
			'ui'			=> 1,
	...
	*/
	...
	function update_value( $value, $post_id, $field ) {
		...
		if( acs_is_sequential_array($value) ) {
			$value = array_map('acs_maybe_idval', $value);
			$value = array_map('strval', $value);
		
		// Parse single value for id.
		} else {
			$value = acs_maybe_idval( $value );
		}
...
// initialize
acs_register_field_type( 'acs_field_page_link' );
