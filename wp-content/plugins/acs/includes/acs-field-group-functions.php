...
// Register store.
acs_register_store( 'field-groups' )->prop( 'multisite', true );
 ...
 */
function acs_get_field_group( $id = 0 ) {
	...
	// Check store.
	$store = acs_get_store( 'field-groups' );
	if( $store->has( $id ) ) {
		return $store->get( $id );
	}
	
	// Check local fields first.
	if( acs_is_local_field_group($id) ) {
		$field_group = acs_get_local_field_group( $id );
	
	// Then check database.
	} else {
		$field_group = acs_get_raw_field_group( $id );
	}
	...
	$field_group = acs_validate_field_group( $field_group );
	 ...
	 */
	$field_group = apply_filters( 'acs/load_field_group', $field_group );
 ...
 * @return	(array|false) The field group array.
 */
function acs_get_raw_field_group( $id = 0 ) {
	
	// Get raw field group from database.
	$post = acs_get_field_group_post( $id );
	if( !$post ) {
		return false;
	}
 ...
 */
function acs_get_field_group_post( $id = 0 ) {
	...
	// Search posts if is string.
		...
		// Try cache.
		$cache_key = acs_cache_key( "acs_get_field_group_post:key:$id" );
		...
		if( $post_id === false ) {
			...
			$posts = get_posts(array(
				'posts_per_page'			=> 1,
				'post_type'					=> 'acs-field-group',
				'post_status'				=> array('publish', 'acs-disabled', 'trash'),
 ...
 * @return	bool
 */
function acs_is_field_group_key( $id = '' ) {
	 ...
	 * @param	string $id The identifier.
	 */
	return apply_filters( 'acs/is_field_group_key', false, $id );
}

/**
 * acs_validate_field_group
 *
 ...
 * @return	array
 */
function acs_validate_field_group( $field_group = array() ) {
	 ...
	 */
	$field_group = apply_filters( 'acs/validate_field_group', $field_group );
 ...
 * @return	array
 */
function acs_get_valid_field_group( $field_group = false ) {
	return acs_validate_field_group( $field_group );
}

/**
 * acs_translate_field_group
 *
 ...
 * @return	array
 */
function acs_translate_field_group( $field_group = array() ) {
	...
	if( $l10n && $l10n_textdomain ) {
		...
		$field_group['title'] = acs_translate( $field_group['title'] );
		 ...
		 * @param	array $field_group The field group array.
		 */
		$field_group = apply_filters( "acs/translate_field_group", $field_group );
	}
...
add_action('acs/validate_field_group', 'acs_translate_field_group');


/**
 * acs_get_field_groups
 *
 ...
 * @return	array
 */
function acs_get_field_groups( $filter = array() ) {
	
	// Vars.
	$field_groups = array();
	
	// Check database.
	$raw_field_groups = acs_get_raw_field_groups();
	if( $raw_field_groups ) {
		foreach( $raw_field_groups as $raw_field_group ) {
			$field_groups[] = acs_get_field_group( $raw_field_group['ID'] );
		}
	}
	 ...
	 */
	$field_groups = apply_filters( 'acs/load_field_groups', $field_groups );
	
	// Filter results.
	if( $filter ) {
		return acs_filter_field_groups( $field_groups, $filter );
	}
 ...
 * @return	array
 */
function acs_get_raw_field_groups() {
	
	// Try cache.
	$cache_key = acs_cache_key( "acs_get_field_group_posts" );
	...
	if( $post_ids === false ) {
		...
		$posts = get_posts(array(
			...
			'update_post_meta_cache'	=> false,
			'update_post_term_cache'	=> false,
			'post_status'				=> array('publish', 'acs-disabled'),
		));
	...
	foreach( $post_ids as $post_id ) {
		$field_groups[] = acs_get_raw_field_group( $post_id );
	}
 ...
 * @return	array
 */
function acs_filter_field_groups( $field_groups, $args = array() ) {
	
	// Loop over field groups and check visibility.
	$filtered = array();
	if( $field_groups ) {
		foreach( $field_groups as $field_group ) {
			if( acs_get_field_group_visibility( $field_group, $args ) ) {
...
/**
 * acs_get_field_group_visibility
 *
 ...
 * @return	bool
 */
function acs_get_field_group_visibility( $field_group, $args = array() ) {
	...
	if( $field_group['location'] ) {
		...
		$screen = acs_get_location_screen( $args );
		...
		foreach( $field_group['location'] as $group ) {
			...
			foreach( $group as $rule ) {
				if( !acs_match_location_rule( $rule, $screen, $field_group ) ) {
					$match_group = false;
					break;
				}
			}
 ...
 * @return	array
 */
function acs_update_field_group( $field_group ) {
	
	// Validate field group.
	$field_group = acs_get_valid_field_group( $field_group );
	
	// May have been posted. Remove slashes.
	$field_group = wp_unslash( $field_group );
	
	// Parse types (converts string '0' to int 0).
	$field_group = acs_parse_types( $field_group );
	...
	acs_extract_vars( $_field_group, array( 'ID', 'key', 'title', 'menu_order', 'fields', 'active', '_valid' ) );
	
	// Create array of data to save.
	$save = array(
		'ID'			=> $field_group['ID'],
    	'post_status'	=> $field_group['active'] ? 'publish' : 'acs-disabled',
    	'post_type'		=> 'acs-field-group',
	...
	acs_flush_field_group_cache( $field_group );
	 ...
	 * @param	array $field_group The field group array.
	 */
	do_action( 'acs/update_field_group', $field_group );
...
/**
 * _acs_apply_unique_field_group_slug
 *
 * Allows full control over 'acs-field-group' slugs.
 *
 * @date	21/1/19
 * @since	5.7.10
 *
 ...
 */
function _acs_apply_unique_field_group_slug( $slug, $post_ID, $post_status, $post_type, $post_parent, $original_slug ) {
...
// Hook into filter.
add_filter( 'wp_unique_post_slug', '_acs_apply_unique_field_group_slug', 999, 6 );

/**
 * acs_flush_field_group_cache
 *
 ...
 * @return	void
 */
function acs_flush_field_group_cache( $field_group ) {
	
	// Delete stored data.
	acs_get_store( 'field-groups' )->remove( $field_group['key'] );
	
	// Flush cached post_id for this field group's key.
	wp_cache_delete( acs_cache_key( "acs_get_field_group_post:key:{$field_group['key']}" ), 'acs' );
	
	// Flush cached array of post_ids for collection of field groups.
	wp_cache_delete( acs_cache_key( "acs_get_field_group_posts" ), 'acs' );
}

/**
 * acs_delete_field_group
 *
 ...
 */
function acs_delete_field_group( $id = 0 ) {
	
	// Disable filters to ensure ACS loads data from DB.
	acs_disable_filters();
	 ...
	 * @param	array $field_group The field group array.
	 */
	do_action( 'acs/delete_field_group', $field_group );
 ...
 */
function acs_trash_field_group( $id = 0 ) {
	 ...
	 */
	do_action( 'acs/trash_field_group', $field_group );
 ...
 */
function acs_untrash_field_group( $id = 0 ) {
	 ...
	 * @param	array $field_group The field_group array.
	 */
	do_action( 'acs/untrash_field_group', $field_group );
...
/**
 * Filter callback which returns the previous post_status instead of "draft" for the "acs-field-group" post type.
 *
 ...
 * @return string.
 */
function _acs_untrash_field_group_post_status( $new_status, $post_id, $previous_status ) {
	return ( get_post_type( $post_id ) === 'acs-field-group' ) ? $previous_status : $new_status;
}

add_action( 'wp_untrash_post_status', '_acs_untrash_field_group_post_status', 10, 3 );
 ...
 * @return	bool
 */
function acs_is_field_group( $field_group = false ) {
	return ( 
 ...
 */
function acs_duplicate_field_group( $id = 0, $new_post_id = 0 ){
	...
	// Duplicate fields.
	$duplicates = acs_duplicate_fields( $fields, $field_group['ID'] );
	
	// Save field group.
	$field_group = acs_update_field_group( $field_group );
	 ...
	 * @param	array $field_group The field_group array.
	 */
	do_action( 'acs/duplicate_field_group', $field_group );
 ...
 * @return	string.
 */
function acs_get_field_group_style( $field_group ) {
	 ...
	 * @param	array $field_group The field group array.
	 */
	return apply_filters('acs/get_field_group_style', $style, $field_group);
}

/**
 * acs_get_field_group_edit_link
 *
 ...
 * @return	string
 */
function acs_get_field_group_edit_link( $post_id ) {
	if( $post_id && acs_current_user_can_admin() ) {
...
/**
 * acs_prepare_field_group_for_export
 *
 ...
 * @return	array
 */
function acs_prepare_field_group_for_export( $field_group = array() ) {

	// Remove args.
	acs_extract_vars( $field_group, array( 'ID', 'local', '_valid' ) );
	
	// Prepare fields.
	$field_group['fields'] = acs_prepare_fields_for_export( $field_group['fields'] );
	 ...
	 * @param	array $field_group The $field group array.
	 */
	return apply_filters( 'acs/prepare_field_group_for_export', $field_group );
}

/**
 * acs_prepare_field_group_for_import
 *
 ...
 * @return	array
 */
function acs_prepare_field_group_for_import( $field_group ) {
	 ...
	 * @param	array $field_group The field group array.
	 */
	return apply_filters( "acs/prepare_field_group_for_import", $field_group );
}

/**
 * acs_import_field_group
 *
 ...
 * @return	array The new field group.
 */
function acs_import_field_group( $field_group ) {
	
	// Disable filters to ensure data is not modified by local, clone, etc.
	$filters = acs_disable_filters();
	...
	$fields = acs_prepare_fields_for_import( $field_group['fields'] );
	...
	if( $field_group['ID'] ) {
		...
		$db_fields = acs_prepare_fields_for_import( acs_get_fields( $field_group ) );
	 ...
	 * @param	array $field_group The field_group array.
	 */
	do_action( 'acs/import_field_group', $field_group );
