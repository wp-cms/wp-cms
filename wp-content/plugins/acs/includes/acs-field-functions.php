...
// Register store.
acs_register_store( 'fields' )->prop( 'multisite', true );
...
function acs_get_field( $id = 0 ) {
	...
	// Check store.
	$store = acs_get_store( 'fields' );
	if( $store->has( $id ) ) {
		return $store->get( $id );
	}
	
	// Check local fields first.
	if( acs_is_local_field($id) ) {
		$field = acs_get_local_field( $id );
	
	// Then check database.
	} else {
		$field = acs_get_raw_field( $id );
	}
	...
	// Validate field.
	$field = acs_validate_field( $field );
	 ...
	 */
	$field = apply_filters( "acs/load_field", $field );
...
// Register variation.
acs_add_filter_variations( 'acs/load_field', array('type', 'name', 'key'), 0 );
 ...
 * @return	(array|false) The field array.
 */
function acs_get_raw_field( $id = 0 ) {
	
	// Get raw field from database.
	$post = acs_get_field_post( $id );
	if( !$post ) {
		return false;
	}
	
	// Bail early if incorrect post type.
	if( $post->post_type !== 'acs-field' ) {
		return false;
	}
 ...
 * @return	(array|false) The field array.
 */
function acs_get_field_post( $id = 0 ) {
	...
	// Search posts if is string.
		...
		// Determine id type.
		$type = acs_is_field_key($id) ? 'key' : 'name';
		
		// Try cache.
		$cache_key = acs_cache_key( "acs_get_field_post:$type:$id" );
		$post_id = wp_cache_get( $cache_key, 'acs' );
		if( $post_id === false ) {
			...
			// Update cache.
			wp_cache_set( $cache_key, $post_id, 'acs' );
		}
...
function acs_is_field_key( $id = '' ) {
	 ...
	 * @param	string $id The identifier.
	 */
	return apply_filters( 'acs/is_field_key', false, $id );
}
 ...
 * @return	array
 */
function acs_validate_field( $field = array() ) {
	 ...
	 * @param	array $field The field array.
	 */
	$field = apply_filters( "acs/validate_field", $field );
...
// Register variation.
acs_add_filter_variations( 'acs/validate_field', array('type'), 0 );
 ...
 * @return	array
 */
function acs_get_valid_field( $field = false ) {
	return acs_validate_field( $field );
}
 ...
 * @return	array
 */
function acs_translate_field( $field = array() ) {
	
	// Get settings.
	$l10n = acs_get_setting('l10n');
	$l10n_textdomain = acs_get_setting('l10n_textdomain');
	
	// Translate field settings if textdomain is set.
	if( $l10n && $l10n_textdomain ) {
		
		$field['label'] = acs_translate( $field['label'] );
		$field['instructions'] = acs_translate( $field['instructions'] );
		 ...
		 * @param	array $field The field array.
		 */
		$field = apply_filters( "acs/translate_field", $field );	
	}
...
// Register variation.
acs_add_filter_variations( 'acs/translate_field', array('type'), 0 );

// Translate fields passing through validation.
add_action('acs/validate_field', 'acs_translate_field');
 ...
 * @return	array
 */
function acs_get_fields( $parent ) {
	
	// Allow field group selector as $parent.
	if( !is_array($parent) ) {
		$parent = acs_get_field_group( $parent );
		if( !$parent ) {
			return array();
		}
	}
	...
	// Check local fields first.
	if( acs_have_local_fields($parent['key']) ) {
		$raw_fields = acs_get_local_fields( $parent['key'] );
		foreach( $raw_fields as $raw_field ) {
			$fields[] = acs_get_field( $raw_field['key'] );
		}
	
	// Then check database.
	} else {
		$raw_fields = acs_get_raw_fields( $parent['ID'] );
		foreach( $raw_fields as $raw_field ) {
			$fields[] = acs_get_field( $raw_field['ID'] );
		}
	}
	 ...
	 */
	$fields = apply_filters( 'acs/load_fields', $fields, $parent );
 ...
 * @return	array
 */
function acs_get_raw_fields( $id = 0 ) {
	
	// Try cache.
	$cache_key = acs_cache_key( "acs_get_field_posts:$id" );
	$post_ids = wp_cache_get( $cache_key, 'acs' );
	if( $post_ids === false ) {
		...
		// Update cache.
		wp_cache_set( $cache_key, $post_ids, 'acs' );
	}
	
	// Loop over ids and populate array of fields.
	$fields = array();
	foreach( $post_ids as $post_id ) {
		$fields[] = acs_get_raw_field( $post_id );
	}
 ...
 * @return	int
 */
function acs_get_field_count( $parent ) {
	 ...
	 * @param	int $count The number of fields.
	  * @param	array $parent The field group or field array.
	 */
	return apply_filters( 'acs/get_field_count', count($raw_fields), $parent );
}
 ...
 * @return	array
 */
function acs_clone_field( $field, $clone_field ) {
	 ...
	 */
	$field = apply_filters( "acs/clone_field", $field, $clone_field );
...
// Register variation.
acs_add_filter_variations( 'acs/clone_field', array('type'), 0 );
 ...
 * @return	array
 */
function acs_prepare_field( $field ) {
	 ...
	 * @param	array $field The field array.
	 */
	$field = apply_filters( "acs/prepare_field", $field );
...
// Register variation.
acs_add_filter_variations( 'acs/prepare_field', array('type', 'name', 'key'), 0 );
 ...
 * @return	void
 */
function acs_render_fields( $fields, $post_id = 0, $el = 'div', $instruction = 'label' ) {
	
	// Parameter order changed in ACS 5.6.9.
	if( is_array($post_id) ) {
		$args = func_get_args();
		$fields = $args[1];
		$post_id = $args[0];
	 ...
	 * @param	(int|string) $post_id The post ID to load values from.
	 */
	$fields = apply_filters( 'acs/pre_render_fields', $fields, $post_id );
	...
	if( $fields ) {
		foreach( $fields as $field ) {
			...
			if( $field['value'] === null ) {
				$field['value'] = acs_get_value( $post_id, $field );
			} 
			
			// Render wrap.
			acs_render_field_wrap( $field, $el, $instruction );
		}
	}
	...
	* @param	(int|string) $post_id The post ID to load values from.
	*/
	do_action( 'acs/render_fields', $fields, $post_id );
}

/**
 * acs_render_field_wrap
 *
 ...
 * @return	void
 */
function acs_render_field_wrap( $field, $element = 'div', $instruction = 'label' ) {
	
	// Ensure field is complete (adds all settings).
	$field = acs_validate_field( $field );
	
	// Prepare field for input (modifies settings).
	$field = acs_prepare_field( $field );
	...
	$wrapper['class'] .= " acs-field-{$field['type']}";
	
	// add field key attributes
	if( $field['key'] ) {
		$wrapper['class'] .= " acs-field-{$field['key']}";
	}
	...
	if( $field['wrapper'] ) {
		$wrapper = acs_merge_attributes( $wrapper, $field['wrapper'] );
	}
	
	// Extract wrapper width and generate style.
	// Todo: Move from $wrapper out into $field.
	$width = acs_extract_var( $wrapper, 'width' );
	if( $width ) {
		$width = acs_numval( $width );
	 ...
	 * @param	array $field The field array.
	 */
	$wrapper = apply_filters( 'acs/field_wrapper_attributes', $wrapper, $field );
	...
	// Vars for render.
	$attributes_html = acs_esc_attr( $wrapper );
	
	// Render HTML
	echo "<$element $attributes_html>" . "\n";
		if( $element !== 'td' ) {
			echo "<$inner_element class=\"acs-label\">" . "\n";
		...
		echo "<$inner_element class=\"acs-input\">" . "\n";
 ...
 * @return	void
 */
function acs_render_field( $field ) {
	 ...
	 * @param	array $field The field array.
	 */
	do_action( "acs/render_field", $field );
}

// Register variation.
acs_add_action_variations( 'acs/render_field', array('type', 'name', 'key'), 0 );

/**
 * acs_render_field_label
 *
 ...
 * @return	void
 */
function acs_render_field_label( $field ) {
	
	// Get label.
	$label = acs_get_field_label( $field );
	
	// Output label.
	if( $label ) {
		echo '<label' . ($field['id'] ? ' for="' . esc_attr($field['id']) . '"' : '' ) . '>' . acs_esc_html($label) . '</label>';
	}
}
 ...
 * @return	void
 */
function acs_get_field_label( $field, $context = '' ) {
	 ...
	 * @param	string $context The output context (admin).
	 */
	$label = apply_filters( "acs/get_field_label", $label, $field, $context );
 ...
 * @return	void
 */
function acs_render_field_instructions( $field ) {
	
	// Output instructions.
	if( $field['instructions'] ) {
		echo '<p class="description">' . acs_esc_html($field['instructions']) . '</p>';
	}
}

/**
 * acs_render_field_setting
 *
 ...
 * @return	void
 */
function acs_render_field_setting( $field, $setting, $global = false ) {
	
	// Validate field.
	$setting = acs_validate_field( $setting );
	
	// Add custom attributes to setting wrapper.
	$setting['wrapper']['data-key'] = $setting['name'];
	$setting['wrapper']['class'] .= ' acs-field-setting-' . $setting['name'];
	if( !$global ) {
	...
	// Render setting.
	acs_render_field_wrap( $setting, 'tr', 'label' );
}
 ...
 * @return	void
 */
function acs_update_field( $field, $specific = array() ) {
	...
	if( $field['parent'] && !is_numeric($field['parent']) ) {
		$parent = acs_get_field_post( $field['parent'] );
	 ...
	 * @param	array $field The field array.
	 */
	$field = apply_filters( "acs/update_field", $field );
	
	// Make a backup of field data and remove some args.
	$_field = $field;
	acs_extract_vars( $_field, array( 'ID', 'key', 'label', 'name', 'prefix', 'value', 'menu_order', 'id', 'class', 'parent', '_name', '_prepare', '_valid' ) );
	...
	if( $specific ) {
		$specific[] = 'ID';
		$save = acs_get_sub_array( $save, $specific );
	}
	 ...
	 * @param	array $field The field array.
	 */
	do_action( "acs/updated_field", $field );	
...
// Register variation.
acs_add_filter_variations( 'acs/update_field', array('type', 'name', 'key'), 0 );

/**
 * _acs_apply_unique_field_slug
 *
 * Allows full control over 'acs-field' slugs.
 *
 * @date	21/1/19
 * @since	5.7.10
 *
 ...
 */
function _acs_apply_unique_field_slug( $slug, $post_ID, $post_status, $post_type, $post_parent, $original_slug ) {
	
	// Check post type and reset to original value.
	if( $post_type === 'acs-field' ) {
...
// Hook into filter.
add_filter( 'wp_unique_post_slug', '_acs_apply_unique_field_slug', 999, 6 );

/**
 * acs_flush_field_cache
 *
 ...
 * @return	void
 */
function acs_flush_field_cache( $field ) {
	
	// Delete stored data.
	acs_get_store( 'fields' )->remove( $field['key'] );
	
	// Flush cached post_id for this field's name and key.
	wp_cache_delete( acs_cache_key("acs_get_field_post:name:{$field['name']}"), 'acs' );
	wp_cache_delete( acs_cache_key("acs_get_field_post:key:{$field['key']}"), 'acs' );
	
	// Flush cached array of post_ids for this field's parent.
	wp_cache_delete( acs_cache_key("acs_get_field_posts:{$field['parent']}"), 'acs' );
}
 ...
 */
function acs_delete_field( $id = 0 ) {
	
	// Get the field.
	$field = acs_get_field( $id );
	 ...
	 * @param	array $field The field array.
	 */
	do_action( "acs/delete_field", $field );
...
// Register variation.
acs_add_action_variations( 'acs/delete_field', array('type', 'name', 'key'), 0 );
...
function acs_trash_field( $id = 0 ) {
	 ...
	 * @param	array $field The field array.
	 */
	do_action( 'acs/trash_field', $field );
 ...
 * @return	bool True if field was trashed.
 */
function acs_untrash_field( $id = 0 ) {
	 ...
	 * @param	array $field The field array.
	 */
	do_action( 'acs/untrash_field', $field );
...
/**
 * Filter callback which returns the previous post_status instead of "draft" for the "acs-field" post type.
 *
 ...
 * @return string.
 */
function _acs_untrash_field_post_status( $new_status, $post_id, $previous_status ) {
	return ( get_post_type( $post_id ) === 'acs-field' ) ? $previous_status : $new_status;
}

add_action( 'wp_untrash_post_status', '_acs_untrash_field_post_status', 10, 3 );
 ...
 * @return	void
 */
function acs_prefix_fields( &$fields, $prefix = 'acs' ) {
	
	// Loopover fields.
	foreach( $fields as &$field ) {
		
		// Replace 'acs' with $prefix.
 ...
 * @return	(array|false)
 */
function acs_get_sub_field( $id, $field ) {
	...
	if( isset($field['sub_fields']) ) {
		$sub_field = acs_search_fields( $id, $field['sub_fields'] );
	}
	 ...
	 * @param	array $field The parent field array.
	 */
	$sub_field = apply_filters( "acs/get_sub_field", $sub_field, $id, $field );
...
// Register variation.
acs_add_filter_variations( 'acs/get_sub_field', array('type'), 2 );
 ...
 * @return	(int|false)
 */
function acs_search_fields( $id, $fields ) {
 ...
 * @return	bool
 */
function acs_is_field( $field = false, $id = '' ) {
	return ( 
		is_array($field)
		&& isset($field['key'])
		&& isset($field['name'])
 ...
 * @return	array
 */
function acs_get_field_ancestors( $field ) {
	
	// Vars.
	$ancestors = array();
	
	// Loop over parents.
	while( $field['parent'] && $field = acs_get_field($field['parent']) ) {
 ...
 * @return	array
 */
function acs_duplicate_fields( $fields = array(), $parent_id = 0 ) {
	...
	acs_append_data( 'generated_keys', $keys );

	// Duplicate fields.
	foreach( $fields as $field ) {
		$field_id = $field['ID'] ? $field['ID'] : $field['key'];
		$duplicates[] = acs_duplicate_field( $field_id, $parent_id );
	}
 ...
 */
function acs_duplicate_field( $id = 0, $parent_id = 0 ){
	...
	// Generate key.
	$keys = acs_get_data( 'generated_keys' );
	 ...
	 * @param	array $field The field array.
	 */
	$field = apply_filters( "acs/duplicate_field", $field);
	
	// Update and return.
	return acs_update_field( $field );
}

// Register variation.
acs_add_filter_variations( 'acs/duplicate_field', array('type'), 0 );

/**
 * acs_prepare_fields_for_export
 *
 ...
 * @return	array
 */
function acs_prepare_fields_for_export( $fields = array() ) {
	
	// Map function and return.
	return array_map( 'acs_prepare_field_for_export', $fields );
}

/**
 * acs_prepare_field_for_export
 *
 ...
 * @return	array
 */
function acs_prepare_field_for_export( $field ) {
	
	// Remove args.
	acs_extract_vars( $field, array( 'ID', 'prefix', 'value', 'menu_order', 'id', 'class', 'parent', '_name', '_prepare', '_valid' ) );
	 ...
	 * @param	array $field The field array.
	 */
	return apply_filters( "acs/prepare_field_for_export", $field );
}

// Register variation.
acs_add_filter_variations( 'acs/prepare_field_for_export', array('type'), 0 );

/**
 * acs_prepare_field_for_import
 *
 ...
 * @return	array
 */
function acs_prepare_fields_for_import( $fields = array() ) {
	...
	while( $i < count($fields) ) {
		...
		// Prepare field.
		$field = acs_prepare_field_for_import( $fields[ $i ] );
	 ...
	 * @param	array $fields The array of fields.
	 */
	return apply_filters( 'acs/prepare_fields_for_import', $fields );
}
 ...
 * @return	array
 */
function acs_prepare_field_for_import( $field ) {
	 ...
	 * @param	array $field The field array.
	 */
	return apply_filters( "acs/prepare_field_for_import", $field );
}

// Register variation.
acs_add_filter_variations( 'acs/prepare_field_for_import', array('type'), 0 );