...
function get_field( $selector, $post_id = false, $format_value = true ) {
	...
	// filter post_id
	$post_id = acs_get_valid_post_id( $post_id );
	
	
	// get field
	$field = acs_maybe_get_field( $selector, $post_id );
	...
	// get value for field
	$value = acs_get_value( $post_id, $field );
	...
	if( $format_value ) {
		...
		// get value for field
		$value = acs_format_value( $value, $post_id, $field );
...
function get_field_object( $selector, $post_id = false, $format_value = true, $load_value = true ) {
	...
	if( $format_value ) {
		...
		// get value for field
		$field['value'] = acs_format_value( $field['value'], $post_id, $field );
...
*  @return	$field (array)
*/
function acs_maybe_get_field( $selector, $post_id = false, $strict = true ) {
	...
	if( acs_is_field_key($selector) ) {
		return acs_get_field( $selector );
	}
	
	// Lookup field via reference.
	$post_id = acs_get_valid_post_id( $post_id );
	$field = acs_get_meta_field( $selector, $post_id );
	if( $field ) {
		return $field;
	}
...
function acs_maybe_get_sub_field( $selectors, $post_id = false, $strict = true ) {
	...
	// vars
	$offset = acs_get_setting('row_index_offset');
	$selector = acs_extract_var( $selectors, 0 );
	$selectors = array_values( $selectors ); // reset keys
	
	
	// attempt get field
	$field = acs_maybe_get_field( $selector, $post_id, $strict );
	...
	for( $j = 0; $j < count($selectors); $j+=2 ) {
		...
		// find sub field
		$field = acs_get_sub_field( $sub_s, $field );
...
function get_field_objects( $post_id = false, $format_value = true, $load_value = true ) {
	...
	foreach( $meta as $key => $value ) {
		...
		// get field
		$field = acs_get_field($meta["_$key"]);
...
function have_rows( $selector, $post_id = false ) {
	...
	$active_loop = acs_get_loop('active');
	$prev_loop = acs_get_loop('previous');
	$new_loop = false;
	$sub_field = false;
	...
	// Detect "change" compared to the active loop.
		...
		$sub_field = acs_get_sub_field($selector, $active_loop['field']);
		if( $sub_field ) {
	...
	if( $new_loop ) {
		...
		if( acs_get_field_type_prop($args['field']['type'], 'have_rows') === 'single' ) {
			$args['value'] = array( $args['value'] );
		}
		
		// Add loop.
		$active_loop = acs_add_loop($args);
	}
...
function get_row( $format = false ) {
	...
	// get value
	$value = acs_maybe_get( $loop['value'], $loop['i'] );
	...
	if( $format ) {
		...
		// single row
		if( acs_get_field_type_prop($field['type'], 'have_rows') === 'single' ) {
			
			// format value
			$value = acs_format_value( $value, $loop['post_id'], $field );
		...
		// multiple rows
			...
			$value = acs_format_value( $loop['value'], $loop['post_id'], $field );
			$value = acs_maybe_get( $value, $loop['i'] );
...
function get_row_sub_field( $selector ) {
	...
	$sub_field = acs_get_sub_field($selector, $row['field']);
...
function get_sub_field_object( $selector, $format_value = true, $load_value = true ) {
	...
	if( $format_value ) {
		...
		// get value for field
		$sub_field['value'] = acs_format_value( $sub_field['value'], $row['post_id'], $sub_field );
...
function get_row_layout() {
	...
	// return
	if( isset($row['acs_fc_layout']) ) {
		
		return $row['acs_fc_layout'];
...
*  acs_shortcode()
*
*  This function is used to add basic shortcode support for the ACS plugin
*  eg. [acs field="heading" post_id="123" format_value="1"]
*
*  @type	function
*  @since	1.1.1
*  @date	29/01/13
...
function update_field( $selector, $value, $post_id = false ) {
	...
	// get field
	$field = acs_maybe_get_field( $selector, $post_id, false );
	...
	// save
	return acs_update_value( $value, $post_id, $field );
...
function update_sub_field( $selector, $value, $post_id = false ) {
	...
	if( is_array($selector) ) {
		...
		$post_id = acs_get_valid_post_id( $post_id );
		$sub_field = acs_maybe_get_sub_field( $selector, $post_id, false );
		
	} else {
		
		$post_id = acs_get_loop('active', 'post_id');
		$sub_field = get_row_sub_field( $selector );
	...
	// update
	return acs_update_value( $value, $post_id, $sub_field );
...
function delete_field( $selector, $post_id = false ) {
	...
	// delete
	return acs_delete_value( $post_id, $field );
...
function add_row( $selector, $row = false, $post_id = false ) {
	...
	// update value
	acs_update_value( $value, $post_id, $field );
...
function add_sub_row( $selector, $row = false, $post_id = false ) {
	...
	// get raw value
	$value = acs_get_value( $post_id, $sub_field );
...
function acs_filter_post_id( $post_id ) {
	
	return acs_get_valid_post_id( $post_id );
