...
class wpdb {
	 ...
	 * @param string      $output Optional. The required return type. One of 'object', 'associative_array', or 'numeric_array', which
	...
	public function get_row( $query = null, $output = 'object', $y = 0 ) {
		...
		} elseif ( 'associative_array' === $output ) {
			return $this->last_result[ $y ] ? get_object_vars( $this->last_result[ $y ] ) : null;
		} elseif ( 'numeric_array' === $output ) {
			return $this->last_result[ $y ] ? array_values( get_object_vars( $this->last_result[ $y ] ) ) : null;
		} else {
			$this->print_error( ' $db->get_row(string query, output type, int offset) -- Output type must be one of: "object", "associative_array", "numeric_array"' );
		}
	}
	 ...
	 * @param string $output Optional. Any of 'associative_array' | 'numeric_array' | 'object' | 'object_key_first_column' strings.
	 *                       With one of the first three, return an array of rows indexed
	 *                       from 0 by SQL result row number. Each row is an associative array
	 *                       (column => value, ...), a numerically indexed array (0 => value, ...),
	 *                       or an object ( ->column = value ), respectively. With 'object_key_first_column',
	...
	public function get_results( $query = null, $output = 'object' ) {
		...
		if ( 'object' === $output ) {
		...
		} elseif ( 'object_key_first_column' === $output ) {
			...
			return $new_array;
		} elseif ( 'associative_array' === $output || 'numeric_array' === $output ) {
			// Return an integer-keyed array of...
			if ( $this->last_result ) {
				foreach ( (array) $this->last_result as $row ) {
					if ( 'numeric_array' === $output ) {
	...
	protected function strip_invalid_text( $data ) {
		...
		if ( $db_check_string ) {
			...
			$this->check_current_query = false;
			$row                       = $this->get_row( 'SELECT ' . implode( ', ', $sql ), 'associative_array' );
			if ( ! $row ) {
