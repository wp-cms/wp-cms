...
switch ( $step ) {
	...
	case 2:
		...
		foreach ( $config_file as $line_num => $line ) {
			...
			if ( ! preg_match( '/^const ([A-Z_]+)([ ]+)=/', $line, $match ) ) {
				continue;
			}
			...
			switch ( $constant ) {
				...
				case 'DB_PASSWORD':
				case 'DB_HOST':
					$config_file[ $line_num ] = 'const ' . $constant . $padding . '= \'' . addcslashes( constant( $constant ), "\\'" ) . "';\r\n";
					break;
				case 'DB_CHARSET':
					if ( 'utf8mb4' === $wpdb->charset || ( ! $wpdb->charset && $wpdb->has_cap( 'utf8mb4' ) ) ) {
						$config_file[ $line_num ] = 'const ' . $constant . $padding . "= 'utf8mb4';\r\n";
					}
					break;
				case 'AUTH_KEY':
				...
				case 'LOGGED_IN_SALT':
				case 'NONCE_SALT':
					$config_file[ $line_num ] = 'const ' . $constant . $padding . '= \'' . $secret_keys[ $key++ ] . "';\r\n";
					break;
			}
		}
		unset( $line );
